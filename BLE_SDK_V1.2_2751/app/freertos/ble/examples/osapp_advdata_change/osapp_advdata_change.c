...
static int32_t osapp_update_advertise_data(uint8_t time)
{
...
{
    test_cnt ++;
    osapp_update_advertise_data(test_cnt);
}
...
static void osapp_gapm_cmp_evt_handler(ke_msg_id_t const msgid, void const *param,ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    ...
    case GAPM_SET_DEV_CONFIG:
        if(cmp_evt->status == GAP_ERR_NO_ERROR)
        {
            ...
            Timer = xTimerCreate("Timer",4,pdTRUE,(void *) 0,osapp_advdata_change_handler);        //50 ticks
            if(Timer != NULL)
                xTimerStart(Timer,0);
        }
        else
