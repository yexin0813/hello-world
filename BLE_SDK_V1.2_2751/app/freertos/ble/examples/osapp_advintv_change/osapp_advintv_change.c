...
#define OSAPP_DFLT_DEVICE_NAME            ("APOLLO_00-ADV")
...
static uint8_t test_cnt;
static TimerHandle_t Timer;


static int32_t osapp_start_advertising(uint8_t time)
{
    ...
    cmd->intv_min = APP_ADV_INT_MIN+time*10;
    cmd->intv_max = APP_ADV_INT_MAX+time*10;
    LOG(LOG_LVL_INFO,"min_%d,max_%d\n",cmd->intv_min,cmd->intv_max);
...
void osapp_advdata_change_handler()
{
    if(test_cnt<5)  
    {
    	test_cnt ++;
    	LOG(LOG_LVL_INFO,"time_cnt = %d\n",test_cnt);
    	xTimerStop(Timer,0);
    	osapp_gapm_cancel();
    }
    else
...
static void osapp_gapc_disconnect_ind_handler(ke_msg_id_t const msgid, void const *param,ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    ...
    osapp_start_advertising(test_cnt);
}
...
static void osapp_gapm_cmp_evt_handler(ke_msg_id_t const msgid, void const *param,ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    ...
    case GAPM_SET_DEV_CONFIG:
        if(cmp_evt->status == GAP_ERR_NO_ERROR)
        {
            ...
            Timer = xTimerCreate("Timer",50,pdTRUE,(void *) 0,osapp_advdata_change_handler);        //50 ticks
            if(Timer != NULL)
                xTimerStart(Timer,0);
        }
        else
    ...
    case GAPM_UPDATE_ADVERTISE_DATA:
        LOG(LOG_LVL_WARN,"adv data update complete\n");
        break;
    default:
