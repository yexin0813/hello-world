...
#include "rwip.h"                // for timing information
...
/// Maximum timer value
#if (BT_EMB_PRESENT)
#define KE_GROSSTIMER_MASK      (BT_GROSSTARGET_MASK)
#elif (BLE_EMB_PRESENT)
#define KE_GROSSTIMER_MASK      (BLE_GROSSTARGET_MASK)
#else
#define KE_GROSSTIMER_MASK      (HL_GROSSTARGET_MASK)
#endif // BT_EMB_PRESENT.

/// Maximum timer value
#define KE_TIMER_DELAY_MAX      (KE_GROSSTIMER_MASK >> 1)
 ...
 ****************************************************************************************
 */
static uint32_t ke_time(void)
{
    #if (BT_EMB_PRESENT || BLE_EMB_PRESENT)
    return ((ea_time_get_halfslot_rounded() >> 4) & KE_GROSSTIMER_MASK);
    #else
    return (timer_get_time() & KE_GROSSTIMER_MASK);
    #endif // BT_EMB_PRESENT
}
...
{
    return (((uint32_t)(newer - older) & KE_GROSSTIMER_MASK) <= KE_TIMER_DELAY_MAX);
}
...
static bool cmp_abs_time(struct co_list_hdr const * timerA, struct co_list_hdr const * timerB)
{
    ...
    return (((uint32_t)(timeA - timeB) & KE_GROSSTIMER_MASK) > KE_TIMER_DELAY_MAX);
}
...
void ke_timer_set(ke_msg_id_t const timer_id, ke_task_id_t const task_id, uint32_t delay)
{
    ...
    struct ke_timer *timer = (struct ke_timer*)
