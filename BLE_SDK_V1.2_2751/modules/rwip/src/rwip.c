...
static uint32_t rwip_slot_2_lpcycles(uint32_t slot_cnt)
{
    ...
    #if (RC32K_USED)
    lpcycles = (slot_cnt<<11)*625/calib32k;
    #else
    #if HZ32000
...
{
    #if HW_BX_VERSION == 00
    sysc_awo_awo_reset_signal_set(SRST_BLE_LP_SET);
    #endif
    ble_lp_intr_clr();
    ble_lp_irq_enable();
...
uint32_t rwip_sleep_lpcycles_2_us(uint32_t lpcycles)
{
    ...
    #if (RC32K_USED)
    rwip_env.sleep_acc_error = lpcycles*calib32k + rwip_env.sleep_acc_error;
