...
static bool h4tl_out_of_sync_check(struct h4tl_env_tag* env)
{
    ...
    if((env-> tl_type == H4TL_TYPE_HCI) && (memcmp(&(hci_reset_msg[0]), &(env->rx_buf[RX_TMP_BUFF_SIZE-HCI_RESET_MSG_LEN]) , HCI_RESET_MSG_LEN) == 0))
    {
        #if(AHI_TL_SUPPORT )
    ...
    if(!sync_ok)
    {
        ...
        if(memcmp(&(ahi_sync_pattern[0]), &(env->rx_buf[RX_TMP_BUFF_SIZE-AHI_RESET_MSG_LEN]) , AHI_RESET_MSG_LEN) == 0)
        {
            sync_ok = true;
        }
    }
...
void h4tl_write(uint8_t type, uint8_t *buf, uint16_t len, void (*tx_callback)(void))
{
    ...
    *buf = type;
    ...
    h4tl_env[tl_type].ext_if->write(buf, len + HCI_TRANSPORT_HDR_LEN, (rwip_eif_callback)&h4tl_tx_done, &(h4tl_env[tl_type]));
}
