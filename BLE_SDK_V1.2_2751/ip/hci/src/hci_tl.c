 ...
 *****************************************************************************************
 */
static uint8_t* hci_build_tci_evt(struct ke_msg * msg)
{
    uint8_t* param = ke_msg2param(msg);
 ...
 *****************************************************************************************
 */
static uint8_t* hci_build_acl_rx_data(struct ke_msg * msg)
{
    // Point to message parameters structure
    struct hci_acl_data_rx *param = (struct hci_acl_data_rx *) ke_msg2param(msg);
    ...
    co_write16p(pk, handle_flags);
    pk +=2;

    // Pack the data length
    co_write16p(pk, param->length);
    pk +=2;
 ...
 *****************************************************************************************
 */
static uint8_t* hci_build_bt_acl_rx_data(struct ke_msg * msg)
{
    struct hci_bt_acl_data_rx *param = (struct hci_bt_acl_data_rx *) ke_msg2param(msg);
 ...
 *****************************************************************************************
 */
static uint8_t* hci_build_bt_sync_rx_data(struct ke_msg * msg)
{
    struct hci_bt_sync_data_rx *param = (struct hci_bt_sync_data_rx *) ke_msg2param(msg);
...
{
    // Point to message parameters structure
    struct hci_acl_data_tx *param = (struct hci_acl_data_tx *) ke_msg2param(msg);
...
static void hci_tx_start(void)
{
    ...
    if (msg == NULL)
    {
        ...
        {
            msg = (struct ke_msg *)co_list_pick(&hci_tl_env.sync_queue);
    ...
    switch(msg->id)
    {
        ...
        case HCI_BT_ACL_DATA_RX:
        {
            ...
            // Extract information (buffer, length, type)
            len = HCI_ACL_HDR_LEN + co_read16p(buf + HCI_ACL_HDR_DATA_LEN_POS);
            type = HCI_ACL_MSG_TYPE;
        }
        break;
        #if VOICE_OVER_HCI
...
static void hci_tx_done(void)
{
    ...
    switch(msg->id)
    {
        ...
        case HCI_ACL_DATA_TX:
        {
            ...
            // free ACL TX buffer
            ke_free(data_tx->buffer - (HCI_ACL_HDR_LEN + HCI_TRANSPORT_HDR_LEN));
        }
        break;
                ...
                case CTRL:
                {
                    ...
                    #endif //BT_EMB_PRESENT
                    {
                        ...
                        if(length >= 2)
                        {
                            ...
                            uint16_t conhdl = co_read16p(payload);

                            #if (BLE_EMB_PRESENT && (HCI_BLE_CON_SUPPORT))
                            // Check if the connection handle corresponds to an active BLE link
                            if((conhdl < BLE_CONNECTION_MAX) && (ke_state_get(KE_BUILD_ID(TASK_LLC, conhdl)) != LLC_FREE))
                            {
                                // Build the destination task ID
...
uint8_t* hci_acl_tx_data_alloc(uint16_t hdl_flags, uint16_t len)
{
    ...
    if((dest != TASK_NONE) && (len <= max_len))
    {
        ...
        if(buf == NULL)
        {
            ...
            buf = ke_malloc(len, KE_MEM_KE_MSG);
        }
    }
    ...
    do
    {
        ...
        if(hci_tl_env.txtag != NULL)
        {
            ...
            if(conhdl < BLE_CONNECTION_MAX)
            {
                ...
                struct hci_acl_data_tx* data_tx = KE_MSG_ALLOC(HCI_ACL_DATA_TX, KE_BUILD_ID(TASK_LLC, conhdl), 0, hci_acl_data_tx);
        ...
        if(hci_tl_env.buf_elt != NULL)
        {
            ...
            if (data_flags & BCF_MASK)
            {
                ...
                struct hci_bt_acl_data_tx* data_tx = KE_MSG_ALLOC(HCI_BT_ACL_DATA_TX, TASK_LB, conhdl, hci_bt_acl_data_tx);
            ...
            {
                // Sends the Kernel message (with space for unpacked parameters)
                struct hci_bt_acl_data_tx* data_tx = KE_MSG_ALLOC(HCI_BT_ACL_DATA_TX, KE_BUILD_ID(TASK_LC, conhdl - BT_ACL_CONHDL_MIN), 0, hci_bt_acl_data_tx);
                hci_tl_env.buf_elt->data_len_flags = (datalen << BT_EM_ACL_DATA_LEN_POS) & BT_EM_ACL_DATA_LEN_MASK;
                hci_tl_env.buf_elt->data_len_flags |= (data_flags << BT_EM_ACL_FLAGS_POS) & BT_EM_ACL_FLAGS_MASK;
                data_tx->buf_elt = hci_tl_env.buf_elt;
                ke_msg_send(data_tx);
...
void hci_sync_tx_data_received(uint16_t conhdl_flags, uint8_t len, uint8_t * payload)
{
    ...
    {
        // Sends the Kernel message (with space for unpacked parameters)
        struct hci_bt_sync_data_tx* data_tx = KE_MSG_ALLOC(HCI_BT_SYNC_DATA_TX, KE_BUILD_ID(TASK_LC, conhdl - BT_ACL_CONHDL_MIN), 0, hci_bt_sync_data_tx);
...
{
    return ke_malloc(len, KE_MEM_NON_RETENTION);
}
...
void hci_acl_rx_data_received(uint16_t hdl_flags, uint16_t datalen, uint8_t * payload)
{
    ...
    {
        // Allocate a Kernel message (with space for unpacked parameters)
        struct hci_acl_data_rx* data_rx = KE_MSG_ALLOC(HCI_ACL_DATA_RX, KE_BUILD_ID(TASK_L2CC, idx), 0, hci_acl_data_rx);
    ...
    switch(code)
    {
        case HCI_CMD_CMP_EVT_CODE:
        {
            if(length >= HCI_CCEVT_HDR_PARLEN)
            {
                ...
                uint16_t opcode = co_read16p(payload + 1);
                // Look for the command descriptor
                const struct hci_cmd_desc_tag* cmd_desc = hci_look_for_cmd_desc(opcode);
                ...
                if(cmd_desc != NULL)
                {
                    ...
                    if(dest != TASK_NONE)
                    {
                        ...
                        // Allocate a Kernel message (with space for unpacked parameters)
                        evt = ke_msg_alloc(HCI_CMD_CMP_EVENT, dest, opcode, unpk_length);
        ...
        case HCI_CMD_STATUS_EVT_CODE:
        {
            if(length == HCI_CSEVT_PARLEN)
            {
                ...
                uint16_t opcode = co_read16p(payload + 2);
                // Look for the command descriptor
                const struct hci_cmd_desc_tag* cmd_desc = hci_look_for_cmd_desc(opcode);
                ...
                if(cmd_desc != NULL)
                {
                    ...
                    {
                        // Allocate a Kernel message (with space for unpacked parameters)
                        void* evt = ke_msg_alloc(HCI_CMD_STAT_EVENT, dest, opcode, length);
        ...
        default:
        {
            ...
            if(evt_desc != NULL)
            {
                ...
                if(dest != TASK_NONE)
                {
                    ...
                    {
                        evt = ke_msg_alloc(HCI_LE_EVENT, dest, 0, unpk_length);
                    }
                    else
                    {
                        evt = ke_msg_alloc(HCI_EVENT, dest, code_or_subcode, unpk_length);
                    }
