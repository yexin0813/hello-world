 ...
 ****************************************************************************************
 */
__INLINE void lld_pdu_push_back(struct co_list *list, struct co_list_hdr *list_hdr)
{
...
__INLINE struct co_list_hdr *lld_pdu_pop_front(struct co_list *list)
{
 ...
 ****************************************************************************************
 */
__INLINE void lld_pdu_push_front(struct co_list *list, struct co_list_hdr *list_hdr)
{
    // check if list is empty
    if (list->first == NULL)
    {
...
static struct llc_llcp_recv_ind* lld_pdu_llcp_unpack(uint8_t rx_hdl, uint8_t len, struct llc_llcp_recv_ind *msg)
{
    ...
    {
        msg->status = CO_ERROR_UNKNOWN_LMP_PDU;
    }
    else if (len != lld_pdu_llcp_pk_desc_tab[msg->pdu.opcode].pdu_len)
    {
        msg->status = CO_ERROR_INVALID_LMP_PARAM; // incorrect length
    }
    else
    {
...
static uint8_t lld_pdu_tx_flush_list(struct co_list *list)
{
    ...
    while (1)
    {
        ...
        GLOBAL_INT_DISABLE();
        txnode = (struct em_desc_node*)co_list_pop_front(list);
        GLOBAL_INT_RESTORE();
...
bool lld_pdu_check(struct lld_evt_tag *evt)
{
    ...
    struct co_list tmp_pdu_list;
    co_list_init(&tmp_pdu_list);
    ...
    GLOBAL_INT_DISABLE();
    txnode = (struct em_desc_node *)co_list_pick(&evt->tx_prog);
    GLOBAL_INT_RESTORE();
    ...
    while (txnode)
    {
        ...
        if (ble_txdone_getf(txnode->idx))
        {
            ...
            txnode = (struct em_desc_node *)co_list_pop_front(&evt->tx_prog);
            if (!co_list_is_empty(&evt->tx_acl_tofree))
            {
                tx_tofree = (struct lld_pdu_data_tx_tag *)co_list_pop_front(&evt->tx_acl_tofree);
                if (tx_tofree)
                {
                    ke_free(tx_tofree);
                }
    ...
    // ***********************************************************************************
    GLOBAL_INT_DISABLE();
    info = (struct lld_pdu_rx_info *)co_list_pop_front(&lld_evt_env.rx_pkt_deferred);
    GLOBAL_INT_RESTORE();
    ...
    while (info != NULL)
    {
        ...
        else
        {
            ...
            co_list_push_back(&tmp_pdu_list, &(info->hdr));
        }
        ...
        if(info == NULL)
        {
            if(!co_list_is_empty(&tmp_pdu_list))
            {
                co_list_merge(&lld_evt_env.rx_pkt_deferred, &tmp_pdu_list);
            }
        }
        GLOBAL_INT_RESTORE();
    }
...
{
    struct co_list *acl_rdy = &evt->tx_acl_rdy;

    GLOBAL_INT_DISABLE();
    // Sanity checks
    ASSERT_ERR(co_list_is_empty(&evt->tx_prog));
    ASSERT_ERR(!co_list_is_empty(acl_rdy));
...
void lld_pdu_data_tx_push(struct lld_evt_tag *evt , struct em_desc_node* txnode, bool can_be_freed, bool encrypted)
{
    ...
    struct co_list *list = &evt->tx_prog;
    ...
    lld_pdu_push_back(list, (struct co_list_hdr *)txnode);
}
#if (BLE_PERIPHERAL || BLE_CENTRAL)
bool lld_pdu_data_send(void *param)
{
    struct lld_pdu_data_tx_tag *data_tx = (struct lld_pdu_data_tx_tag*)ke_malloc(sizeof(struct lld_pdu_data_tx_tag), KE_MEM_ENV);
    ...
    if(data_tx)
    {
        ...
        // Get list of data packets ready for programming
        struct co_list *list = &evt->tx_acl_rdy;
        ...
        co_list_push_back(list, &data_tx->hdr);
        GLOBAL_INT_RESTORE();
        status = true;
    }
    return (status);
...
void lld_pdu_tx_prog(struct lld_evt_tag *evt)
{
    ...
    #if (BLE_CENTRAL || BLE_PERIPHERAL)
    struct co_list *llcp_pdu_rdy    = &evt->tx_llcp_pdu_rdy;
    #endif // (BLE_CENTRAL || BLE_PERIPHERAL)

    struct co_list *tx_prog         = &evt->tx_prog;
    uint16_t tx_add                 = 0;
    bool send_acl                   = true;
    struct em_desc_node *txnode = NULL;
    ...
    if (!co_list_is_empty(llcp_pdu_rdy) && ble_txdone_getf(conhdl))
    {
        // Pick the 1st llcp pdu in TX llcp pending list
        struct llcp_pdu_tag *llcp_elt = (struct llcp_pdu_tag *)co_list_pick(llcp_pdu_rdy);
        ...
        if(llcp_pdu_authorized)
        {
            ...
            co_list_extract(llcp_pdu_rdy, &llcp_elt->hdr, 0);
            ...
            if(status == LLC_PDU_PACK_OK)
            {
                ...
                co_list_push_back(tx_prog, &llcp_elt->hdr);
                evt->evt.conn.tx_prog_pkt_cnt++;
            }
        }// if(llcp_pdu_authorized)
    }
    ...
    if (!co_list_is_empty(acl_rdy) && (send_acl))
    {
        #if (BLE_CENTRAL || BLE_PERIPHERAL)
        struct lld_pdu_data_tx_tag *txacl = NULL;
        if((evt->conhdl < BLE_CONNECTION_MAX) )
        {
            uint16_t nb_tx_desc_available   = co_list_size(&em_buf_env.tx_desc_free);
...
void lld_pdu_tx_flush(struct lld_evt_tag *evt)
{
    ...
    while(!co_list_is_empty(list))
    {
        struct llcp_pdu_tag *llcp_elt = (struct llcp_pdu_tag *)co_list_pop_front(list);
        ke_free(llcp_elt->ptr);
        ke_free(llcp_elt);
    }
    GLOBAL_INT_RESTORE();
...
static void lld_pdu_llcp_rx_handler(struct lld_pdu_rx_info* info, uint8_t length)
{
    ...
    if(llc_env[info->conhdl]->llcp_pass_through_enable)
    {
        ...
        struct hci_dbg_ble_tst_llcp_recv_evt *event = KE_MSG_ALLOC(HCI_DBG_EVT, info->conhdl, 0, hci_dbg_ble_tst_llcp_recv_evt);
        // gets event subcode
    ...
    #endif // (BLE_TESTER)
    {
        ke_task_id_t llc_taskid      = KE_BUILD_ID(TASK_LLC, info->conhdl);

        // Allocate message pdu handler
        struct llc_llcp_recv_ind *msg = KE_MSG_ALLOC(LLC_LLCP_RECV_IND, llc_taskid, llc_taskid, llc_llcp_recv_ind);
...
void lld_pdu_rx_handler(struct lld_evt_tag *evt, uint8_t nb_rx_desc)
{
    ...
    while(nb_rx_desc--)
    {
        ...
        {
            struct lld_pdu_rx_info* info = ke_malloc(sizeof(struct lld_pdu_rx_info), KE_MEM_KE_MSG);
            #if (BLE_CENTRAL || BLE_PERIPHERAL)
                    ...
                    && (info->length > 0))
                {
                    ...
                    switch(llid)
                    {
                        ...
                        case LLID_CONTINUE:
                        {
                            ...
                            // security checks
                            struct llc_data_ind *ll_data = KE_MSG_ALLOC(LLC_DATA_IND, llc_taskid, llc_taskid, llc_data_ind);
            ...
            co_list_push_back(&lld_evt_env.rx_pkt_deferred, &info->hdr);
        }
        else
        {
            // Free the received descriptor
