...
static const struct ke_task_desc TASK_DESC_LLD = {NULL, NULL, NULL, 1, 1};
...
void lld_init(bool reset)
{
    ...
    ble_bdaddrl_set(co_read32p(&bd_addr.addr[0]));
    ble_bdaddru_set(co_read16p(&bd_addr.addr[4]));
        ...
        uint8_t adv_pwr)
{
	...
	if(jump_table[HBEACON]) Hbeacon_trap(HBEACON_START,adv_par,NULL,NULL);
...
{
    if(jump_table[HBEACON]) Hbeacon_trap(HBEACON_STOP,NULL,NULL,NULL);
        ...
        uint16_t conhdl)
{
    ...
    if((elt_scan != NULL)&&(elt_connect != NULL))
    {
        ...
        elt_scan->timestamp = CO_ALIGN2_HI(ea_time_get_halfslot_rounded() + elt_scan->start_latency + RWBLE_ASAP_LATENCY) & BLE_BASETIMECNT_MASK;
        ...
        if(ea_offset_req(&input_param, &set_param) != EA_ERROR_REJECTED)
        {
            ...
            // gets the crc init
            co_write16 (&data.crcinit.crc[0],co_htobs(0x0000FFFF & random_nb));
            co_write8 (&data.crcinit.crc[2],(0x00FF0000 & random_nb)>>16);
            // gets the latency
            data.latency = co_htobs(con_par->con_latency);
            // gets the TO
            data.timeout = co_htobs(con_par->superv_to);
            ...
            // Init interval
            data.interval = co_htobs(set_param.interval >> 1);
            // Init window size
            data.winsize = 2;
                        ...
                        struct llcp_con_upd_ind *param_pdu)
{
    ...
    struct ea_elt_tag *elt  = lld_evt_update_create(elt_old,
                                                    co_max(2, CO_ALIGN2_HI(param->ce_len_min)),
                                       ...
                                       bool bypass_offchk)
{
    ...
    if((status == CO_ERROR_NO_ERROR) || (bypass_offchk))
    {
