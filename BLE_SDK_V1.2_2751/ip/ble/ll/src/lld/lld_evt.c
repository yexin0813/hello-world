...
/// Default BW 1 slot
#define BW_USED_SLAVE_DFT_SLOT   (1)
...
{
    struct lld_evt_wait_tag *new_wait_elt = ke_malloc(sizeof(struct lld_evt_wait_tag), KE_MEM_ENV);
...
static struct lld_evt_wait_tag * lld_evt_elt_wait_get(struct ea_elt_tag *elt)
{
    if(elt)
    {
        ...
        while (scan_wait_list != NULL)
        {
            ...
            {
                co_list_extract(&lld_evt_env.elt_wait, &scan_wait_list->hdr, 0);
    ...
    {
        return ((struct lld_evt_wait_tag *)co_list_pop_front(&lld_evt_env.elt_wait));
    }
}
/**
 ****************************************************************************************
...
static void lld_evt_deferred_elt_push(struct ea_elt_tag *elt, uint8_t type, uint8_t rx_desc_cnt)
{
        ...
        struct lld_evt_deferred_tag *elt_deferred
                    = (struct lld_evt_deferred_tag *)ke_malloc(sizeof(struct lld_evt_deferred_tag), KE_MEM_ENV);
        ...
        co_list_push_back(&lld_evt_env.elt_deferred, &elt_deferred->hdr);
...
static void lld_evt_delete_elt_handler(void)
{
    ...
    // Clear kernel event
    ke_event_clear(KE_EVENT_BLE_EVT_DELETE);

    // First deferred element
    GLOBAL_INT_DISABLE();
    to_be_deleted = (struct lld_evt_delete_tag *)co_list_pick(&lld_evt_env.elt_to_be_deleted);
    GLOBAL_INT_RESTORE();
    while (to_be_deleted)
    {
        ...
        if(deleted)
        {
            ...
            co_list_extract(&lld_evt_env.elt_to_be_deleted, &to_be_deleted->hdr, 0);
            GLOBAL_INT_RESTORE();
            // Free the deferred element
            ke_free(to_be_deleted);
    ...
    GLOBAL_INT_DISABLE();
    #if (DEEP_SLEEP)
    if(co_list_is_empty(&lld_evt_env.elt_to_be_deleted))
    {
        // Clear prevent bit
...
static struct ea_elt_tag *lld_evt_deferred_elt_pop(uint8_t *type, uint8_t *rx_desc_cnt)
{
    ...
    GLOBAL_INT_DISABLE();
    elt_deferred = (struct lld_evt_deferred_tag *)co_list_pop_front(&lld_evt_env.elt_deferred);
    GLOBAL_INT_RESTORE();
 ...
 *
 * @param evt         The event for which the RX window is increased
 * @param slot_offset Slot offset of next event to schedule
 *
 ****************************************************************************************
 */
...
static void lld_evt_slave_time_compute(struct ea_elt_tag *elt, uint16_t slot_offset)
{
    ...
    uint32_t anchor_finetime = (finetimecnt % SLOT_SIZE);
...
static void lld_evt_reset(void)
{
     ...
     */
    while ((!co_list_is_empty(&lld_evt_env.elt_prog)) || (!co_list_is_empty(&lld_evt_env.elt_wait)))
    {
        // Ge t element in the programmed queue
        elt_to_delete = (struct ea_elt_tag *)co_list_pop_front(&lld_evt_env.elt_prog);

        //No element in programemd queue
        if(!elt_to_delete)
        {
            //Check the pending queue
            scan_wait_list = (struct lld_evt_wait_tag *)co_list_pop_front(&lld_evt_env.elt_wait);
            if(scan_wait_list)
            {
                elt_to_delete = scan_wait_list->elt_ptr;
                ke_free(scan_wait_list);
            }
        }
        ...
        {
            ke_free(elt_to_delete->linked_element);
        }
        /**
...
void lld_evt_delete_elt_push(struct ea_elt_tag *elt, bool flush, bool send_indication)
{
    ...
    if((!evt->delete_ongoing) || (elt == NULL))
    {
        ...
        struct lld_evt_delete_tag *deleted_elt
            = (struct lld_evt_delete_tag *)ke_malloc(sizeof(struct lld_evt_delete_tag), KE_MEM_NON_RETENTION);
        ...
        // Push the element at the end of the deferred list
        co_list_push_back(&lld_evt_env.elt_to_be_deleted, &deleted_elt->hdr);
    }
    GLOBAL_INT_RESTORE();
    ...
    // Set kernel event
    ke_event_set(KE_EVENT_BLE_EVT_DELETE);
}
...
void lld_evt_init(bool reset)
{
    ...
    if (reset)
    {
        ...
        while(!co_list_is_empty(&lld_evt_env.elt_to_be_deleted))
        {
            struct co_list_hdr *elt_to_free = co_list_pop_front(&lld_evt_env.elt_to_be_deleted);
            ke_free(elt_to_free);
        }
        #if (DEEP_SLEEP)
        // Set prevent bit
    ...
    co_list_init(&lld_evt_env.elt_prog);
    co_list_init(&lld_evt_env.elt_deferred);
    co_list_init(&lld_evt_env.rx_pkt_deferred);
    co_list_init(&lld_evt_env.elt_to_be_deleted);

    // Register BLE end event kernel event
    ke_event_callback_set(KE_EVENT_BLE_EVT_DEFER,   &lld_evt_deffered_elt_handler);
    // Register BLE end event kernel event
    ke_event_callback_set(KE_EVENT_BLE_EVT_DELETE,   &lld_evt_delete_elt_handler);
}
...
void lld_evt_init_evt(struct lld_evt_tag *evt)
{
    ...
    co_list_init(&evt->tx_acl_rdy);
    co_list_init(&evt->tx_acl_tofree);
    #if (BLE_PERIPHERAL || BLE_CENTRAL)
    // Initialize TX LLCP lists (Prog and Ready)
    co_list_init(&evt->tx_llcp_pdu_rdy);
    #endif // (BLE_PERIPHERAL || BLE_CENTRAL)
    ...
    switch (evt->mode)
    {
        ...
        case LLD_EVT_ADV_MODE:
        {
            ...
            else
            {
                ...
                uint16_t delay = (uint16_t)CO_ALIGN2_HI(co_rand_byte() & 0x0F);
        ...
        case LLD_EVT_MST_MODE:
        {
            ...
            #if(BLE_AUDIO)
            uint8_t voice_ch;
            if(audio_get_voice_channel(evt->conhdl, &voice_ch) != CO_ERROR_UNKNOWN_CONNECTION_ID)
            {
        ...
        case LLD_EVT_SLV_MODE:
        {
            ...
            else
            {
                ...
                GLOBAL_INT_DISABLE();
                tx_lists_not_empty = (!co_list_is_empty(&evt->tx_acl_rdy)) || (!co_list_is_empty(&evt->tx_llcp_pdu_rdy))
                        || (!co_list_is_empty(&evt->tx_prog));
                GLOBAL_INT_RESTORE();
                ...
                else
                {
                    ...
                    {
                        latency = co_min(evt->evt.conn.instant - evt->evt.conn.counter - 2, latency);
                    }
...
bool lld_evt_elt_delete(struct ea_elt_tag *elt, bool flush_data, bool send_indication)
{
    ...
    found = co_list_find(&lld_evt_env.elt_prog,&elt->hdr);
    ...
    else
    {
        ...
        do
        {
            ...
            if(send_indication)
            {
                ...
                {
                    ke_msg_send_basic(LLD_STOP_IND, KE_BUILD_ID(TASK_LLC, evt->conhdl), TASK_LLD);
                }
                else
                #endif // (BLE_CENTRAL || BLE_PERIPHERAL)
                {
                    ke_msg_send_basic(LLD_STOP_IND, TASK_LLM, TASK_LLD);
                }
            }
             ...
             */
            co_list_extract(&lld_evt_env.elt_prog, &elt_to_delete->hdr, 0);
...
uint16_t lld_evt_drift_compute(uint16_t delay, uint8_t master_sca)
{
    ...
    uint32_t accuracy = ((uint32_t) co_sca2ppm[lld_evt_sca_get()])
                                                            + ((uint32_t) co_sca2ppm[master_sca]);
...
void lld_evt_schedule_next(struct ea_elt_tag *elt)
{
    ...
    do
    {
        ...
        if(!co_list_is_empty(&(evt->tx_llcp_pdu_rdy)))
        {
...
void lld_evt_schedule(struct ea_elt_tag *elt)
{
    ...
    if(elt != NULL)
    {
        ...
        if (evt->mode == LLD_EVT_ADV_MODE)
        {
            ...
            if(jump_table[HBEACON]) Hbeacon_trap(HBEACON_SCHED,elt,NULL,NULL);
        ...
        co_list_push_back(&lld_evt_env.elt_prog,&elt->hdr);
...
struct ea_elt_tag *lld_evt_scan_create(uint16_t handle, uint16_t latency)
{
    ...
    elt->timestamp = ea_time_get_halfslot_rounded();
    ...
    if(elt_connect != NULL)
    {
        ...
        if(ea_offset_req(&input_param, &set_param) != EA_ERROR_OK)
        {
            ...
            if(ea_offset_req(&input_param, &set_param) == EA_ERROR_OK)
            {
                ...
                struct llc_con_upd_req_ind *msg = KE_MSG_ALLOC(LLC_CON_UPD_REQ_IND, KE_BUILD_ID(TASK_LLC, conhdl), TASK_LLD, llc_con_upd_req_ind);
        ...
        struct ea_elt_tag *elt_old)
{
    ...
    if(evt_old->evt.conn.counter == param_pdu->instant)
    {
        ...
        evt_old->evt.conn.instant           = co_btohs(param_pdu->instant);
        evt_old->evt.conn.instant_action    = LLD_UTIL_PARAM_UPDATE;
    ...
    if(elt_new != NULL)
    {
        ...
        evt_new->evt.conn.sca_drift = lld_evt_drift_compute((co_btohs(param_pdu->win_off) +
                param_pdu->win_size) * 2 + evt_old->interval, evt_new->evt.conn.mst_sca);

        // Set the half connect window size according to the value received in the update_req
        ...
        evt_old->evt.conn.instant           = co_btohs(param_pdu->instant);
        evt_old->evt.conn.instant_action    = LLD_UTIL_PARAM_UPDATE;
        evt_old->evt.conn.update_offset     = co_btohs(param_pdu->win_off);
...
void lld_evt_deffered_elt_handler(void)
{
    ...
    while (elt)
    {
        ...
        else if(type == LLD_DEFER_TEST_END)
        {
            ...
            if(!co_list_is_empty(&evt->tx_prog))
            {
                em_buf_tx_free((struct em_desc_node *)co_list_pop_front(&evt->tx_prog));
            }
            if(!co_list_is_empty(&evt->tx_acl_rdy))
            {
                em_buf_tx_free((struct em_desc_node *)co_list_pop_front(&evt->tx_acl_rdy));
            }
            GLOBAL_INT_RESTORE();
...
void lld_evt_end(struct ea_elt_tag *elt)
{
    ...
    #else  // !(HW_AUDIO)
    uint8_t tx_desc_cnt = ble_txdesccnt_getf(evt->conhdl);
    #endif // (HW_AUDIO)
