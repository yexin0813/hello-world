...
{
    // Get the BD address
    co_write32p (&bd_addr->addr[0], co_btohl(ble_bdaddrl_get()));
    co_write16p (&bd_addr->addr[4], co_btohs(ble_bdaddru_get()));
}

void lld_util_set_bd_address(struct bd_addr *bd_addr, uint8_t type)
{
    // Set the BD address
    ble_bdaddrl_set(co_htobl(co_read32p(&bd_addr->addr[0])));
    ble_bdaddru_setf(co_htobs(co_read16p(&bd_addr->addr[4])));
...
void lld_util_anchor_point_move(struct ea_elt_tag *elt_connect)
{
    ...
    if(ea_offset_req(&input_param, &set_param) == EA_ERROR_OK)
    {
        ...
        struct llc_con_upd_req_ind *msg = KE_MSG_ALLOC(LLC_CON_UPD_REQ_IND, KE_BUILD_ID(TASK_LLC, evt_connect->conhdl), TASK_LLD, llc_con_upd_req_ind);
...
void lld_util_flush_list(struct co_list *list)
{
    while (1)
    {
        ...
        void *elt = (void  *)co_list_pop_front(list);
...
void lld_util_phy_update_req(struct ea_elt_tag *elt, uint16_t instant, uint8_t tx_phy, uint8_t rx_phy)
{
    ...
    evt->evt.conn.instant           = co_btohs(instant);
