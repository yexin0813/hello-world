...
extern int llc_hci_command_handler(ke_msg_id_t const msgid, void const *param,
                               ke_task_id_t const dest_id, ke_task_id_t const src_id);
extern int llc_hci_acl_data_tx_handler(ke_msg_id_t const msgid,  struct hci_acl_data_tx const *param,
                                   ke_task_id_t const dest_id, ke_task_id_t const src_id);
 ...
 ****************************************************************************************
 */
static int llc_enc_mgt_ind_handler(ke_msg_id_t const msgid,
                                        void const *param,
                ...
                case LLC_REM_WAIT_TP_FOR_ENC_REQ:
                {
                    ...
                    else
                    {
                        ...
                        if (LLC_UTIL_ENC_STATE_IS_SET(conhdl, LLC_ENC_REFRESH_PENDING))
                        {
                            ...
                            llc_llcp_terminate_ind_pdu_send(conhdl, CO_ERROR_PIN_MISSING);
                        }
                        else
                        {
                            ...
                            llc_llcp_reject_ind_pdu_send(conhdl, LLCP_ENC_REQ_OPCODE, CO_ERROR_PIN_MISSING);
 ...
 ****************************************************************************************
 */
static int llc_link_sup_to_ind_handler(ke_msg_id_t const msgid,
                                       void const *param,
                                       ke_task_id_t const dest_id,
                                       ke_task_id_t const src_id)
{
    ...
    {
        llc_util_dicon_procedure(conhdl, CO_ERROR_CON_TIMEOUT);
    }
 ...
 ****************************************************************************************
 */
static int llc_auth_payl_nearly_to_ind_handler(ke_msg_id_t const msgid,
                                               void const *param,
                                               ke_task_id_t const dest_id,
                                               ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        if (!ke_timer_active(LLC_LLCP_RSP_TO, dest_id))
        {
 ...
 ****************************************************************************************
 */
static int llc_auth_payl_real_to_ind_handler(ke_msg_id_t const msgid,
                                             void const *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id)
{
    ...
    {
        ke_timer_set(LLC_AUTH_PAYL_NEARLY_TO, dest_id, llc_env[conhdl]->auth_payl_to
 ...
 ****************************************************************************************
 */
static int llc_llcp_rsp_to_ind_handler(ke_msg_id_t const msgid,
                                       void const *param,
                                       ke_task_id_t const dest_id,
                                       ke_task_id_t const src_id)
{
    ...
    llc_util_dicon_procedure(conhdl, CO_ERROR_LMP_RSP_TIMEOUT);
 ...
 ****************************************************************************************
 */
static int llc_chnl_assess_timer_handler(ke_msg_id_t const msgid,
                                         void const *param,
                                         ke_task_id_t const dest_id,
                                         ke_task_id_t const src_id)
{
    ...
    else if(llc_env_ptr->loc_proc_state == LLC_LOC_WAIT_MAP_UPD_INSTANT)
    {
        ...
        ke_timer_set(LLC_CHNL_ASSESS_TO, dest_id, llm_util_ch_assess_get_assess_timer());
    }
    else
    {
        ...
        ke_msg_send_basic(LLC_CHMAP_UPDATE_REQ_IND, dest_id, dest_id);
 ...
 ****************************************************************************************
 */
static int llc_con_upd_req_ind_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int llc_length_req_ind_handler(ke_msg_id_t const msgid,
                                      void const *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int llc_phy_upd_req_ind_handler(ke_msg_id_t const msgid,
        ...
        switch(param->operation)
        {
            ...
            case LLC_PHY_UPD_PEER_RSP:
            {
                        ...
                        && ((tx_phy == llcp.m_to_s_phy) || (llcp.m_to_s_phy == PHYS_NO_PREF)))
                {
                    ...
                    // Get the
                    // Set state
                    llc_phy_update_finished(conhdl , tx_phy , rx_phy, CO_ERROR_NO_ERROR,phy_local_update->operation);
                    //clear operation
            ...
            case LLC_PHY_UPD_PEER_REQ:
            {
                ...
                if(llc_env[conhdl]->loc_proc_state == LLC_LOC_PHY_NO_INSTANT)
                {
                    //clear operation
                    llc_util_clear_operation_ptr(conhdl, LLC_OP_LOC_PHY_UPD);
                    llc_phy_update_finished(conhdl , tx_phy , rx_phy, CO_ERROR_NO_ERROR, LLC_PHY_UPD_PEER_REQ);
                }
            }break;
 ...
 ****************************************************************************************
 */
static int llc_chmap_update_req_ind_handler(ke_msg_id_t const msgid,
                                      void const *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int llm_enc_ind_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int lld_stop_ind_handler(ke_msg_id_t const msgid,
                                void const *param,
                                ke_task_id_t const dest_id,
                                ke_task_id_t const src_id)
{
    if (ke_state_get(dest_id) != LLC_FREE)
    {
        ...
        if (state != LLC_DISC_BUSY)
        {
            ...
            reason = (GETF(llc_env[conhdl]->llc_status, LLC_STAT_SYNC_FOUND))?CO_ERROR_CON_TIMEOUT:CO_ERROR_CONN_FAILED_TO_BE_EST;;
        }
        ...
        llc_discon_event_complete_send(dest_id, CO_ERROR_NO_ERROR, conhdl, reason);

        // Clear pending timers
        ke_timer_clear(LLC_LE_LINK_SUP_TO, dest_id);
        ke_timer_clear(LLC_LLCP_RSP_TO, dest_id);
        ke_timer_clear(LLC_AUTH_PAYL_NEARLY_TO, dest_id);
        ke_timer_clear(LLC_AUTH_PAYL_REAL_TO, dest_id);
 ...
 ****************************************************************************************
 */
static int llc_data_ind_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int llc_dft_handler(ke_msg_id_t const msgid,
                           void *param,
                           ke_task_id_t const dest_id,
                           ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int llc_llcp_recv_ind_handler(ke_msg_id_t const msgid,
...
static const struct ke_msg_handler llc_default_state[] =
{
    // Note: first message is latest message checked by kernel so default is put on top.
    {KE_MSG_DEFAULT_HANDLER,    (ke_msg_func_t)llc_dft_handler},
     ...
     * ************** Msg HCI->LLC ****************
     */
    {HCI_COMMAND,               (ke_msg_func_t)llc_hci_command_handler},
    // Data sent to the peer
    {HCI_ACL_DATA_TX,           (ke_msg_func_t)llc_hci_acl_data_tx_handler},

    /*
     * ************** Msg LLC->HL *****************
     */
    // Data received from the peer
    {LLC_DATA_IND,              (ke_msg_func_t)llc_data_ind_handler},
    ...
    {LLC_AUTH_PAYL_NEARLY_TO,   (ke_msg_func_t)llc_auth_payl_nearly_to_ind_handler},
    // Authenticated Payload Timeout
    {LLC_AUTH_PAYL_REAL_TO,     (ke_msg_func_t)llc_auth_payl_real_to_ind_handler},
    #endif //#if !(BLE_QUALIF)
    // Response Timeout
    {LLC_LLCP_RSP_TO,           (ke_msg_func_t)llc_llcp_rsp_to_ind_handler},
    #if (BLE_CHNL_ASSESS)
    // Channel assessment timer expiration
    {LLC_CHNL_ASSESS_TO,        (ke_msg_func_t)llc_chnl_assess_timer_handler},
    #endif //(BLE_CHNL_ASSESS)
    // A pause resume mechanism is on going
    {LLC_ENC_MGT_IND,           (ke_msg_func_t)llc_enc_mgt_ind_handler},
    #if !(BLE_QUALIF)
    // indication that a length_req is needed
    {LLC_LENGTH_REQ_IND,        (ke_msg_func_t)llc_length_req_ind_handler},
    #endif //#if !(BLE_QUALIF)
    // indication that a length_req is needed
    {LLC_CON_UPD_REQ_IND,       (ke_msg_func_t)llc_con_upd_req_ind_handler},
    #if (BLE_CHNL_ASSESS)
    // Channel assessment timer expiration
    {LLC_CHMAP_UPDATE_REQ_IND,  (ke_msg_func_t)llc_chmap_update_req_ind_handler},
    #endif //(BLE_CHNL_ASSESS)
    #if ((BLE_2MBPS) && !(BLE_QUALIF))
    {LLC_PHY_UPD_REQ_IND,       (ke_msg_func_t)llc_phy_upd_req_ind_handler},
    #endif //(BLE_2MBPS)

    /*
     * ************** LLCP messages **************
     */
    {LLC_LLCP_RECV_IND,         (ke_msg_func_t)llc_llcp_recv_ind_handler},

    /*
     * ************** Msg LLM->LLC **************
     */
    // Data has been encrypted by the AES
    {LLM_ENC_IND,               (ke_msg_func_t)llm_enc_ind_handler},
};


/// Specifies the message handlers that are common to all states.
const struct ke_state_handler llc_default_handler = KE_STATE_HANDLER(llc_default_state);
