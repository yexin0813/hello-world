...
typedef int (*llcp_pdu_handler_func_t)(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, union llcp_pdu *pdu);


static int llc_llcp_unknown_ind_handler(uint16_t conhdl, ke_task_id_t dest_id, uint8_t opcode);
...
#if (BLE_2MBPS)
static int llcp_phy_req_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_phy_req *param);
static int llcp_phy_rsp_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_phy_rsp *param);
static int llcp_phy_upd_ind_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_phy_upd_req *param);
#endif // (BLE_2MBPS)
#endif // !(BLE_QUALIF)
/*
...
void llc_llcp_ch_map_update_pdu_send(uint16_t conhdl)
{
    ...
    pdu.instant = co_htobs(lld_util_instant_get((void*)LLD_EVT_ENV_ADDR_GET(llc_env[conhdl]->elt), (uint8_t)LLD_UTIL_CHMAP_UPDATE));
...
void llc_llcp_pause_enc_req_pdu_send(uint16_t conhdl)
{
    ...
    ke_task_id_t taskid = KE_BUILD_ID(TASK_LLC, conhdl);

    pdu.opcode = LLCP_PAUSE_ENC_REQ_OPCODE;
    // Start the response timeout
    ke_timer_set(LLC_LLCP_RSP_TO, taskid, LLC_DFT_RSP_TO);
...
void llc_llcp_enc_req_pdu_send(uint16_t conhdl, struct hci_le_start_enc_cmd *param)
{
    ...
    co_write16p(&pdu.ediv, co_htobs(param->enc_div));          // EDIV
    ...
    // Start the response timeout
    ke_timer_set(LLC_LLCP_RSP_TO, KE_BUILD_ID(TASK_LLC, conhdl), LLC_DFT_RSP_TO);
...
void llc_llcp_con_update_pdu_send(uint16_t conhdl, struct llcp_con_upd_ind *param)
{
    ...
    pdu.win_off  = co_htobs(param->win_off);
    pdu.interv   = co_htobs(param->interv);
    pdu.latency  = co_htobs(param->latency);
    pdu.timeout  = co_htobs(param->timeout);
    pdu.instant  = co_htobs(param->instant);
...
void llc_llcp_con_param_req_pdu_send(uint16_t conhdl, struct llc_con_upd_req_ind *param)
{
    ...
    pdu.interval_min =  co_htobs(param->interval_min);
    pdu.interval_max =  co_htobs(param->interval_max);
    pdu.latency = co_htobs(param->con_latency);
    pdu.timeout = co_htobs(param->superv_to);
    pdu.pref_period = param->pref_period;
    pdu.ref_con_event_count = co_htobs(param->ref_con_event_count);
    pdu.offset0 = co_htobs(param->offset0);
    pdu.offset1 = co_htobs(param->offset1);
    pdu.offset2 = co_htobs(param->offset2);
    pdu.offset3 = co_htobs(param->offset3);
    pdu.offset4 = co_htobs(param->offset4);
    pdu.offset5 = co_htobs(param->offset5);
...
void llc_llcp_terminate_ind_pdu_send(uint16_t conhdl, uint8_t err_code)
{
    ...
    ke_task_id_t taskid = KE_BUILD_ID(TASK_LLC, conhdl);
    ke_state_t state = ke_state_get(taskid);
    ...
    switch(err_code)
    {
        ...
        {
            llc_env_ptr->disc_reason = CO_ERROR_CON_TERM_BY_LOCAL_HOST;
        }break;
        default:
        {
    ...
    ke_timer_set(LLC_LLCP_RSP_TO, taskid, llc_env_ptr->sup_to);
}
...
void llc_llcp_length_req_pdu_send(uint16_t conhdl)
{
    ...
    pdu.max_rx_octets = co_htobs(llc_env_ptr->data_len_ext_info.conn_max_rx_octets);
    pdu.max_rx_time = co_htobs(llc_env_ptr->data_len_ext_info.conn_max_rx_time);
    pdu.max_tx_octets = co_htobs(llc_env_ptr->data_len_ext_info.conn_max_tx_octets);
    pdu.max_tx_time = co_htobs(llc_env_ptr->data_len_ext_info.conn_max_tx_time);
 ...
 ****************************************************************************************
 */
static void llc_llcp_reject_ind(uint16_t conhdl, ke_task_id_t const dest_id, uint8_t err_code, bool extended)
{
    ...
    ke_timer_clear(LLC_LLCP_RSP_TO, dest_id);
    if(llc_state_chk(state, LLC_LOC_PROC_BUSY))
    {
        ...
        switch (llc_env_ptr->loc_proc_state)
        {
            ...
            case LLC_LOC_WAIT_CON_PARAM_RSP:
            {
                ...
                ke_timer_clear(LLC_LLCP_RSP_TO, dest_id);
                #if (BLE_CENTRAL)
                if ((err_code == CO_ERROR_UNSUPPORTED_REMOTE_FEATURE) && (lld_get_mode(conhdl) == LLD_EVT_MST_MODE))
                {
                    // Get associated BLE Event environment
            ...
            case LLC_LOC_WAIT_PHY_UPD_REQ:
            {
                ...
                {
                    struct llc_phy_upd_req_ind * phy_update = KE_MSG_ALLOC(LLC_PHY_UPD_REQ_IND, KE_BUILD_ID(TASK_LLC, evt->conhdl), KE_BUILD_ID(TASK_LLC, evt->conhdl), llc_phy_upd_req_ind);
    ...
    if (!is_expected)
    {
        if (LLC_UTIL_ENC_STATE_IS_SET(conhdl, LLC_ENC_RX_FLOW_OFF))
        {
            ...
            llc_util_dicon_procedure(conhdl, CO_ERROR_TERMINATED_MIC_FAILURE);
        }
    }
}
...
{
    int msg_status = int_ctx ? KE_MSG_SAVED : KE_MSG_CONSUMED;
    uint8_t state  = ke_state_get(dest_id);
    ...
    {
        uint8_t conhdl = KE_IDX_GET(dest_id);
        struct llc_env_tag *llc_env_ptr = llc_env[conhdl];
        ...
        // unknown pdu
        else if ((status == CO_ERROR_UNKNOWN_LMP_PDU) && !int_ctx)
        {
...
static int llcp_con_upd_ind_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_con_upd_ind *param)
{
    ...
    if(!int_ctx)
    {
        ...
        switch(msg->dummy)
        {
            ...
            {
                llc_llcp_reject_ind_pdu_send(conhdl, param->opcode, CO_ERROR_LMP_COLLISION);
            } break;
            case LLC_INSTANT_REJECT:
            {
                llc_llcp_reject_ind_pdu_send(conhdl, param->opcode, CO_ERROR_UNSUPPORTED);
            } break;
...
static int llcp_terminate_ind_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_terminate_ind *param)
{
    ...
    ke_state_set(dest_id, LLC_DISC_BUSY);
...
static int llcp_feats_req_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_feats_req *param)
{
    ...
    #endif // (BLE_TESTER)
    {
        ...
        else
        {
            ...
            for(uint8_t idx=0 ; idx < LE_FEATS_LEN ; idx++)
            {
...
static int llcp_feats_rsp_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_feats_rsp *param)
{
            ...
            && (llc_env_ptr->loc_proc_state == LLC_LOC_WAIT_FEAT_RSP))
    {
        ...
        ke_timer_clear(LLC_LLCP_RSP_TO, taskid);
        ...
        llc_feats_rd_event_send(CO_ERROR_NO_ERROR, conhdl, &param->feats);
...
static int llcp_con_param_req_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_con_param_req *param)
{
    ...
    else
    {
        ...
        if (lld_get_mode(conhdl) == LLD_EVT_MST_MODE)
        {
            ...
            if(llc_state_chk(state, LLC_LOC_PROC_BUSY))
            {
                switch(llc_env_ptr->loc_proc_state)
                {
                    ...
                    case LLC_LOC_WAIT_MAP_UPD_INSTANT:
                    {
                        llc_llcp_reject_ind_pdu_send(conhdl, param->opcode, CO_ERROR_DIFF_TRANSACTION_COLLISION);
                        reject = true;
                    } break;
        ...
        if(!reject)
        {
            ...
            {
                llc_llcp_reject_ind_pdu_send(conhdl, LLCP_CONNECTION_PARAM_REQ_OPCODE, CO_ERROR_INVALID_LMP_PARAM);
            }
                     ...
                     || (param->timeout      != llc_env_ptr->sup_to))
            {
                ...
                if(llm_util_check_evt_mask(LE_REM_CON_PARA_REQ_EVT_BIT))
                {
                    ...
                    struct llc_con_upd_req_ind *req_param = KE_MSG_ALLOC(LLC_CON_UPD_REQ_IND, dest_id, dest_id, llc_con_upd_req_ind);
                ...
                {
                    llc_llcp_reject_ind_pdu_send(conhdl, LLCP_CONNECTION_PARAM_REQ_OPCODE, CO_ERROR_UNSUPPORTED_REMOTE_FEATURE);
                }
            }
            ...
            {   // Request handled by the controller
                ...
                else if (lld_get_mode(conhdl) == LLD_EVT_SLV_MODE)
                {
                    ...
                    if (lld_con_param_rsp(conhdl, llc_env_ptr->elt, &data) == CO_ERROR_NO_ERROR)
                    {
                    ...
                    {
                        llc_llcp_reject_ind_pdu_send(conhdl, LLCP_CONNECTION_PARAM_REQ_OPCODE, CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE);
                    }
                }
            }
        }
...
static int llcp_con_param_rsp_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_con_param_rsp *param)
{
    ...
    {
        llc_llcp_reject_ind_pdu_send(conhdl, param->opcode, CO_ERROR_UNSPECIFIED_ERROR);
    }
    else
    {
        // LLC is READY to receive commands, process it immediately
        ...
        if (lld_get_mode(conhdl) == LLD_EVT_MST_MODE)
        {
            ...
            if ( (param->interval_min <= req_param->con_intv_max) && (param->interval_max >= req_param->con_intv_min))
            {
                ...
                req_param->con_intv_min = param->interval_min;
                req_param->con_intv_max = param->interval_max;
                req_param->con_latency  = co_max(param->latency, req_param->con_latency) ;
                req_param->superv_to    = co_max(param->timeout, req_param->superv_to);
                req_param->ce_len_min   = llc_env_ptr->elt->duration_min/SLOT_SIZE;
                req_param->ce_len_max   = llc_env_ptr->elt->duration_min/SLOT_SIZE;
            ...
            else
            {
                ...
                llc_llcp_reject_ind_pdu_send(conhdl, LLCP_CONNECTION_PARAM_RSP_OPCODE, CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE);
                ...
                if (GETF(llc_env_ptr->llc_status, LLC_STAT_UPDATE_HOST_REQ))
                {
                    ...
                    {
                        // Send the LE META event connection update to the host
                        llc_con_update_complete_send(CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE, conhdl, evt);
                    }
                }
...
static int llcp_start_enc_rsp_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_start_enc_rsp *param)
{
    ...
    else
    {
        ...
        if (LLC_UTIL_ENC_STATE_IS_SET(conhdl, LLC_ENC_REFRESH_PENDING))
        {
            ...
            llc_common_enc_key_ref_comp_evt_send(conhdl, CO_ERROR_NO_ERROR);
        }
        else
        {
            // Notify the host that the connection is now encrypted
            llc_common_enc_change_evt_send(conhdl, 1, CO_ERROR_NO_ERROR);
        }
    }
...
static int llcp_length_req_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_length_req *param)
{
    ...
    else
    {
        ...
        temp_conn_eff_max_rx_octets = co_min(llc_env_ptr->data_len_ext_info.conn_max_rx_octets,param->max_tx_octets);
        temp_conn_eff_max_rx_time = co_min(llc_env_ptr->data_len_ext_info.conn_max_rx_time,param->max_tx_time);
        temp_conn_eff_max_tx_octets = co_min(llc_env_ptr->data_len_ext_info.conn_max_tx_octets,param->max_rx_octets);
        temp_conn_eff_max_tx_time = co_min(llc_env_ptr->data_len_ext_info.conn_max_tx_time,param->max_rx_time);
...
static int llcp_phy_req_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_phy_req *param)
{
    ...
    else
    {
        ...
        {
            //Values out of range
            llc_llcp_reject_ind_pdu_send(conhdl, param->opcode, CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE);
            reject = true;
        }
        ...
        if (!reject)
        {
            ...
            {
                llc_llcp_reject_ind_pdu_send(conhdl, param->opcode, CO_ERROR_UNSUPPORTED_REMOTE_FEATURE);
            }
        }
    }
...
static int llcp_phy_rsp_handler(uint16_t conhdl, ke_task_id_t dest_id, bool int_ctx, struct llcp_phy_rsp *param)
{
    ...
    else
    {
        ...
        else
        {
            llc_llcp_reject_ind_pdu_send(conhdl, LLCP_CONNECTION_PARAM_RSP_OPCODE, CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE);
            phy_update->operation   = LLC_PHY_UPD_TERMINATE;
            phy_update->status      = CO_ERROR_UNSUPPORTED_LMP_PARAM_VALUE;
    ...
    if(msg->dummy == LLC_INSTANT_IGNORED)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                phy_update->status      = CO_ERROR_NO_ERROR;
                ke_msg_send(phy_update);
        ...
        switch (llc_env_ptr->loc_proc_state)
        {
            ...
            case LLC_LOC_WAIT_ENC_RSP:
            {
                ...
                if (!(LLC_UTIL_ENC_STATE_IS_SET(conhdl, LLC_ENC_REFRESH_PENDING)))
                {
                    ...
                    // Notify the host of the Encryption Start Failure
                    llc_common_enc_change_evt_send(conhdl, 0, CO_ERROR_UNSUPPORTED_REMOTE_FEATURE);
            ...
            case LLC_LOC_WAIT_FEAT_RSP:
            {
                if (param->unk_type == LLCP_SLAVE_FEATURE_REQ_OPCODE)
                {
                    ...
                    llc_feats_rd_event_send(CO_ERROR_UNSUPPORTED_REMOTE_FEATURE, conhdl, &llc_env_ptr->feats_used);
                    ...
                    #endif // (BLE_CENTRAL)
                    {
                        ...
                        if (GETF(llc_env_ptr->llc_status, LLC_STAT_UPDATE_HOST_REQ))
                        {
                            ...
                            if (llm_util_check_evt_mask(LE_CON_UPD_EVT_BIT))
                            {
                                ...
                                struct lld_evt_tag *evt = LLD_EVT_ENV_ADDR_GET(llc_env_ptr->elt);
                                // Send the LE META event connection update to the host
                                llc_con_update_complete_send(CO_ERROR_UNSUPPORTED_REMOTE_FEATURE, conhdl, evt);
                            }
                        }
            ...
            case LLC_LOC_WAIT_PHY_UPD_REQ:
            {
                ...
                ke_timer_clear(LLC_LLCP_RSP_TO, dest_id);
                {
                    ...
                    phy_update->rx_phys     =  evt->evt.conn.rx_phy;
                    phy_update->tx_phys     = evt->evt.conn.tx_phy;
                    phy_update->status      = CO_ERROR_UNSUPPORTED_REMOTE_FEATURE;
                    ke_msg_send(phy_update);
                }
