...
/// LLC task descriptor
static const struct ke_task_desc TASK_DESC_LLC = {NULL, &llc_default_handler, llc_state, LLC_STATE_MAX, LLC_IDX_MAX};
...
void llc_init(void)
{
    ...
    for (conhdl=0; conhdl < BLE_CONNECTION_MAX; conhdl++)
    {
        ...
        ke_state_set(KE_BUILD_ID(TASK_LLC, conhdl), LLC_FREE);
    }
...
void llc_reset(void)
{
    ...
    for (conhdl=0; conhdl < BLE_CONNECTION_MAX; conhdl++)
    {
        if(ke_state_get(KE_BUILD_ID(TASK_LLC, conhdl))!= LLC_FREE)
        {
...
void llc_start(struct llc_create_con_req_ind *param, struct ea_elt_tag *elt)
{
    ...
    ke_timer_set(LLC_LE_LINK_SUP_TO, llc_id, llc_env_ptr->sup_to);
    ...
    llc_env_ptr->disc_reason = CO_ERROR_CON_TIMEOUT;
    ...
    if (llc_env[conhdl] != NULL)
    {
        ...
        for (uint8_t i = 0; i < LLC_OP_MAX; i++)
        {
            if (llc_env[conhdl]->operation[i])
            {
                ...
                ke_msg_free(ke_param2msg(llc_env[conhdl]->operation[i]));
            }
        }

        // Free the LLC environment structure for the link
        ke_free((void *)llc_env[conhdl]);
...
void llc_le_con_cmp_evt_send(uint8_t status, uint16_t conhdl, struct llc_create_con_req_ind *param)
{
    ...
    if(llm_util_check_evt_mask(LE_CON_CMP_EVT_BIT) || llm_util_check_evt_mask(LE_ENH_CON_CMP_EVT_BIT))
    {
        ...
        if(!llm_util_check_evt_mask(LE_ENH_CON_CMP_EVT_BIT))
        {
            ...
            struct hci_le_con_cmp_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, conhdl, 0, hci_le_con_cmp_evt);
            // gets event subcode
            ...
            if(status == CO_ERROR_NO_ERROR)
            {
                ...
                event->con_interval = co_htobs(param->con_int);
                // gets connection latency
                event->con_latency = co_htobs(param->con_lat);
                // gets supervision time out
                event->sup_to = co_htobs(param->sup_to);
                // gets clock accuracy
            ...
            else if (status == CO_ERROR_DIRECT_ADV_TO)
            {
                // gets the role
        ...
        {
            // allocate the status event message
            struct hci_le_enh_con_cmp_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, conhdl, 0, hci_le_enh_con_cmp_evt);
            // gets event subcode
...
void llc_con_update_complete_send(uint8_t status, uint16_t conhdl, struct lld_evt_tag *evt)
{
    ...
    {
        // Gets connection interval
        event->con_interval = co_htobs(evt->interval >> 1);
        // Gets connection latency
        event->con_latency  = co_htobs(evt->evt.conn.latency - 1);
        // Gets link supervision timeout
        event->sup_to       = co_htobs(llc_env[conhdl]->sup_to);
    }
    else
    {
        ...
        if(param != NULL)
        {
            ...
            event->sup_to       = co_htobs(param->superv_to);
        }
        else
        {
            event->con_interval = 0;
...
void llc_ltk_req_send(uint16_t conhdl, struct llcp_enc_req const *param)
{
    ...
    event->ediv = co_read16p(param->ediv);
    // gets random number
                             ...
                             struct le_features const *feats)
{
    ...
    if (llm_util_check_evt_mask(LE_CON_RD_REM_FEAT_EVT_BIT))
    {
        ...
        // allocate the status event message
        struct hci_le_rd_rem_used_feats_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, conhdl, 0, hci_le_rd_rem_used_feats_cmd_cmp_evt);
        // gets event subcode
...
void llc_version_rd_event_send(uint8_t status, uint16_t conhdl)
{
    ...
    event->compid = co_btohs(llc_env[conhdl]->peer_version.compid);
    // gets the sub-version
    event->subvers = co_btohs(llc_env[conhdl]->peer_version.subvers);
    // gets the version
...
void llc_common_flush_occurred_send(uint16_t conhdl)
{
    ...
    struct hci_flush_occurred_evt *event = KE_MSG_ALLOC(HCI_EVENT, conhdl, HCI_FLUSH_OCCURRED_EVT_CODE, hci_flush_occurred_evt);
    event->conhdl= conhdl;
    hci_send_2_host(event);
}
...
{
    // allocates the message to send
    struct hci_enc_key_ref_cmp_evt *event = KE_MSG_ALLOC(HCI_EVENT, conhdl, HCI_ENC_KEY_REFRESH_CMP_EVT_CODE, hci_enc_key_ref_cmp_evt);
    event->conhdl= conhdl;
    event->status= status;
    hci_send_2_host(event);
}
...
void llc_common_enc_change_evt_send(uint16_t conhdl, uint8_t enc_status, uint8_t status)
{
    ...
    if (enc_status)
    {
        ke_timer_set(LLC_AUTH_PAYL_NEARLY_TO, llc_id, llc_env[conhdl]->auth_payl_to - llc_env[conhdl]->auth_payl_to_margin);
        ke_timer_set(LLC_AUTH_PAYL_REAL_TO, llc_id, llc_env[conhdl]->auth_payl_to);
    }
    else
    {
    ...
    #endif // !(BLE_QUALIF)
    // allocates the message to send
    struct hci_enc_change_evt *event = KE_MSG_ALLOC(HCI_EVENT, conhdl, HCI_ENC_CHG_EVT_CODE, hci_enc_change_evt);
    event->conhdl= co_htobs(conhdl);
...
void llc_common_nb_of_pkt_comp_evt_send(uint16_t conhdl, uint8_t nb_of_pkt)
{
    ...
    event->conhdl[0] = co_htobs(conhdl);
    // gets the number of packet sent
    event->nb_comp_pkt[0] = co_htobs(nb_of_pkt);
...
void llc_con_update_ind(uint16_t conhdl, struct ea_elt_tag *elt_new)
{
    ...
    if (GETF(llc_env[conhdl]->llc_status, LLC_STAT_UPDATE_HOST_REQ) || GETF(llc_env[conhdl]->llc_status, LLC_STAT_UPDATE_EVT_SENT))
    {
        ...
        {
            // Send the command complete event
            llc_con_update_complete_send(CO_ERROR_NO_ERROR, conhdl, LLD_EVT_ENV_ADDR_GET(elt_new));
        }
    }
...
void llc_lsto_con_update(uint16_t conhdl)
{
    ...
    uint16_t to = co_max(llc_env_ptr->sup_to, llc_env_ptr->n_sup_to) << 1;
    // update supervision timeout with a temporary value
    llc_env_ptr->sup_to = to;

    // Force restart the link supervision timeout
    ke_timer_set(LLC_LE_LINK_SUP_TO, KE_BUILD_ID(TASK_LLC, conhdl), to);
...
void llc_map_update_ind(uint16_t conhdl)
{
    ...
    ble_chmap0_set(conhdl,co_read16p(&llc_env_ptr->n_ch_map.map[0]));
    ble_chmap1_set(conhdl,co_read16p(&llc_env_ptr->n_ch_map.map[2]));
...
void llc_con_update_finished(uint16_t conhdl)
{
    ...
    ke_task_id_t task_id = KE_BUILD_ID(TASK_LLC, conhdl);
...
{
    // Allocate the status event message
    struct hci_le_phy_update_cmp_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, conhdl, 0, hci_le_phy_update_cmp_evt);
