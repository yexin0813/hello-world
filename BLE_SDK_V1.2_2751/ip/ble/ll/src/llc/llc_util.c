...
static void llc_check_trafic_paused(uint8_t conhdl)
{
    ...
    if(GETF(llc_env[conhdl]->llc_status, LLC_STAT_WAIT_TRAFFIC_PAUSED))
    {
        ...
        if(tx_pkt_cnt == 0)
        {
            ...
            ke_msg_send_basic(LLC_ENC_MGT_IND,task_id,task_id);
        }
    }
}
...
{
    for (uint16_t idx = 0; idx < BLE_CONNECTION_MAX; idx++)
    {
        // Build a task ID dedicated to the conhdl
        ke_task_id_t llc_free = KE_BUILD_ID(TASK_LLC, idx);
...
void llc_util_dicon_procedure(uint16_t conhdl, uint8_t reason)
{
    ...
    ke_state_set(KE_BUILD_ID(TASK_LLC, conhdl), LLC_DISC_BUSY);
    #if (BLE_CHNL_ASSESS)
    // Stop timer
    ke_timer_clear(LLC_CHNL_ASSESS_TO, KE_BUILD_ID(TASK_LLC, conhdl));
    #endif
...
void llc_end_evt_defer(uint16_t conhdl)
{
    ...
    if (llc_env[conhdl] != NULL)
    {
        ...
        if(llc_state_chk(ke_state_get(task_id), LLC_DISC_BUSY) && GETF(llc_env[conhdl]->llc_status, LLC_STAT_DISC_REM_REQ))
        {
...
void llc_pdu_llcp_tx_ack_defer(uint16_t conhdl, uint8_t opcode)
{
    ...
    uint16_t task_id = KE_BUILD_ID(TASK_LLC, conhdl);
    ...
    else
    {
        ...
        if(llc_state_chk(ke_state_get(task_id), LLC_DISC_BUSY))
        {
...
void llc_pdu_defer(uint16_t conhdl, uint16_t status, uint8_t rssi, uint8_t channel, uint8_t length)
{
    ...
    if (llc_env[conhdl] != NULL)
    {
        ...
        else
        {
            ...
            ke_timer_set(LLC_LE_LINK_SUP_TO, KE_BUILD_ID(TASK_LLC, conhdl), (llc_env[conhdl]->sup_to));
            ...
            {
                ke_timer_set(LLC_AUTH_PAYL_NEARLY_TO, task_id, llc_env[conhdl]->auth_payl_to - llc_env[conhdl]->auth_payl_to_margin);
                ke_timer_set(LLC_AUTH_PAYL_REAL_TO, task_id, llc_env[conhdl]->auth_payl_to);
            }
            #endif // !(BLE_QUALIF)
        }
