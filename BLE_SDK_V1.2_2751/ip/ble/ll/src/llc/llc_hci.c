...
static int hci_le_con_update_cmd_handler(ke_msg_id_t const msgid,
                                         struct hci_le_con_update_cmd const *param,
                                         ke_task_id_t const dest_id,
                                         ke_task_id_t const src_id)
{
    uint8_t status = CO_ERROR_COMMAND_DISALLOWED;
    uint8_t complete_status = CO_ERROR_UNDEFINED;
    ...
    else
    {
        ...
        {
            complete_status = CO_ERROR_COMMAND_DISALLOWED;
        }
        else
        #endif //(BLE_QUALIF)
        #if (BLE_PERIPHERAL)
        ...
        {
            complete_status = CO_ERROR_UNSUPPORTED_REMOTE_FEATURE;
        }
        else
        ...
        {
            complete_status = CO_ERROR_NO_ERROR;
        }
        else
        {
            struct llc_con_upd_req_ind * con_update = KE_MSG_ALLOC(LLC_CON_UPD_REQ_IND, dest_id, dest_id, llc_con_upd_req_ind);
    ...
    if(msg_status != KE_MSG_SAVED)
    {
        ...
        if (llm_util_check_evt_mask(LE_CON_UPD_EVT_BIT) && (complete_status != CO_ERROR_UNDEFINED))
        {
 ...
 ****************************************************************************************
 */
static int hci_le_rd_chnl_map_cmd_handler(ke_msg_id_t const msgid,
                                          ...
                                          ke_task_id_t const src_id)
{
    ...
    event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, param->conhdl, HCI_LE_RD_CHNL_MAP_CMD_OPCODE,
 ...
 ****************************************************************************************
 */
static int hci_le_rd_rem_used_feats_cmd_handler(ke_msg_id_t const msgid,
                                                ...
                                                ke_task_id_t const src_id)
{
    ...
    if(msg_status == KE_MSG_CONSUMED)
    {
        ...
        if (GETF(llc_env[param->conhdl]->llc_status, LLC_STAT_FEAT_EXCH))
        {
            ...
            llc_feats_rd_event_send(CO_ERROR_NO_ERROR, param->conhdl, &(llc_env[param->conhdl]->feats_used));
        }
    }
 ...
 ****************************************************************************************
 */
static int hci_le_start_enc_cmd_handler(ke_msg_id_t const msgid,
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if(llc_state_chk(state, LLC_DISC_BUSY))
    {
        llc_common_cmd_status_send(src_id, CO_ERROR_COMMAND_DISALLOWED, param->conhdl);
        // Nothing to do
    }
    // check if traffic is paused
    else if(llc_state_chk(state, LLC_TRAFFIC_PAUSED_BUSY))
    {
        llc_common_cmd_status_send(src_id, CO_ERROR_CONTROLLER_BUSY, param->conhdl);
    ...
    else
    {
        ...
        else
        {
            ...
            // Send the command status event
            llc_common_cmd_status_send(src_id, CO_ERROR_NO_ERROR, param->conhdl);
            ...
            else
            {
                ...
                ke_msg_send_basic(LLC_ENC_MGT_IND,dest_id,dest_id);
            }
 ...
 ****************************************************************************************
 */
static int hci_le_ltk_req_reply_cmd_handler(ke_msg_id_t const msgid,
                                            ...
                                            ke_task_id_t const src_id)
{
    ...
    else if(llc_state_chk(state, LLC_REM_PROC_BUSY))
    {
        if(llc_env[param->conhdl]->rem_proc_state == LLC_REM_WAIT_LTK)
        {
            ...
            struct llm_enc_req* msg = KE_MSG_ALLOC(LLM_ENC_REQ, TASK_LLM, dest_id, llm_enc_req);
 ...
 ****************************************************************************************
 */
static int hci_le_ltk_req_neg_reply_cmd_handler(ke_msg_id_t const msgid,
                                                struct hci_basic_conhdl_cmd const *param,
                                                ke_task_id_t const dest_id,
                                                ke_task_id_t const src_id)
{
    ...
    else if(llc_state_chk(state, LLC_REM_PROC_BUSY))
    {
        if(llc_env[param->conhdl]->rem_proc_state == LLC_REM_WAIT_LTK)
        {
            ...
            if (LLC_UTIL_ENC_STATE_IS_SET(param->conhdl, LLC_ENC_REFRESH_PENDING))
            {
                ...
                llc_llcp_terminate_ind_pdu_send(param->conhdl, CO_ERROR_PIN_MISSING);
            }
            else
            {
                ...
                llc_llcp_reject_ind_pdu_send(param->conhdl, LLCP_ENC_REQ_OPCODE, CO_ERROR_PIN_MISSING);
 ...
 ****************************************************************************************
 */
static int hci_le_rem_con_param_req_reply_cmd_handler(ke_msg_id_t const msgid,
    ...
    else if(llc_state_chk(state, LLC_REM_PROC_BUSY))
    {
        ...
        if(llc_env_ptr->rem_proc_state == LLC_REM_WAIT_CON_PARAM_HOST_RSP)
        {
            ...
            if (lld_get_mode(param->conhdl) == LLD_EVT_MST_MODE)
            {
                ...
                param_req->interval_min = co_min(param_req->interval_min, param->interval_min);
                param_req->interval_max = co_max(param_req->interval_max, param->interval_max);
                param_req->con_latency  = param->latency;
                param_req->superv_to    = co_max(param_req->superv_to,param->timeout);
                ke_msg_send(param_req);
 ...
 ****************************************************************************************
 */
static int hci_le_rem_con_param_req_neg_reply_cmd_handler(ke_msg_id_t const msgid,
                                                ...
                                                ke_task_id_t const src_id)
{
    ...
    else if(llc_state_chk(state, LLC_REM_PROC_BUSY))
    {
        ...
        {
            if(param->reason != CO_ERROR_UNACCEPTABLE_CONN_INT)
            {
                status = CO_ERROR_INVALID_HCI_PARAM;
            }
            else
 ...
 ****************************************************************************************
 */
static int hci_flush_cmd_handler(ke_msg_id_t const msgid,
                                 struct hci_basic_conhdl_cmd const *param,
                                 ke_task_id_t const dest_id,
                                 ke_task_id_t const src_id)
{
    ...
    struct hci_flush_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, param->conhdl, HCI_FLUSH_CMD_OPCODE, hci_flush_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_disconnect_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_rd_rssi_cmd_handler(ke_msg_id_t const msgid,
                                   struct hci_basic_conhdl_cmd const *param,
                                   ke_task_id_t const dest_id,
                                   ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_rd_tx_pwr_lvl_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_rd_rem_ver_info_cmd_handler(ke_msg_id_t const msgid,
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    else if(GETF(llc_env[conhdl]->llc_status, LLC_STAT_PEER_VERS_KNOWN))
    {
        ...
        llc_version_rd_event_send(CO_ERROR_NO_ERROR, conhdl);
    }
    // check if local procedure is on-going
    else if(llc_state_chk(state, LLC_LOC_PROC_BUSY)
            // check if traffic is paused
 ...
 ****************************************************************************************
 */
static int hci_rd_auth_payl_to_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_wr_auth_payl_to_cmd_handler(ke_msg_id_t const msgid,
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        if ((uint32_t)(param->auth_payl_to*16) >= (uint32_t)(evt->interval * evt->evt.conn.latency))
        {
            ...
            {
                ke_timer_set(LLC_AUTH_PAYL_NEARLY_TO, dest_id, llc_env[conhdl]->auth_payl_to - llc_env[conhdl]->auth_payl_to_margin);
                ke_timer_set(LLC_AUTH_PAYL_REAL_TO, dest_id, llc_env[conhdl]->auth_payl_to);
            }
            #endif //!(BLE_QUALIF)
        }
        else
 ...
 ****************************************************************************************
 */
static int hci_tester_set_le_params_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_dbg_ble_tst_llcp_pt_en_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_dbg_ble_tst_send_llcp_cmd_handler(ke_msg_id_t const msgid,
 ...
 ***************************************************************************************
*/
static int hci_le_set_data_len_cmd_handler(ke_msg_id_t const msgid,
    ...
    else
    {
        ...
        else
        {
                    ...
                    ||(llc_env_ptr->data_len_ext_info.conn_eff_max_tx_time != param->tx_time))
            {
                 ...
                 */
                ke_msg_send_basic(LLC_LENGTH_REQ_IND, dest_id, dest_id);
 ...
 ****************************************************************************************
 */
static int hci_le_rd_phy_cmd_handler(ke_msg_id_t const msgid,
                                           struct hci_le_rd_phy_cmd const *param,
                                           ke_task_id_t const dest_id,
                                           ke_task_id_t const src_id)
{
    // allocate the status event message
    struct hci_le_rd_phy_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, param->conhdl, HCI_LE_RD_PHY_CMD_OPCODE, hci_le_rd_phy_cmd_cmp_evt);
    // Get current state
    uint8_t state = ke_state_get(dest_id);
    // check if state is Free or in disconnected state
    if(llc_state_chk(state, LLC_DISC_BUSY))
 ...
 ****************************************************************************************
 */
static int hci_le_set_phy_cmd_handler(ke_msg_id_t const msgid,
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    else
    {
        ...
        {
            struct llc_phy_upd_req_ind * phy_update = KE_MSG_ALLOC(LLC_PHY_UPD_REQ_IND, dest_id, dest_id, llc_phy_upd_req_ind);
...
static const struct ke_msg_handler llc_hci_command_handler_tab[] =
{
    {HCI_DISCONNECT_CMD_OPCODE                      , (ke_msg_func_t)hci_disconnect_cmd_handler},
    {HCI_RD_REM_VER_INFO_CMD_OPCODE                 , (ke_msg_func_t)hci_rd_rem_ver_info_cmd_handler},
    {HCI_FLUSH_CMD_OPCODE                           , (ke_msg_func_t)hci_flush_cmd_handler},
    {HCI_RD_TX_PWR_LVL_CMD_OPCODE                   , (ke_msg_func_t)hci_rd_tx_pwr_lvl_cmd_handler},
    {HCI_RD_RSSI_CMD_OPCODE                         , (ke_msg_func_t)hci_rd_rssi_cmd_handler},
    {HCI_RD_AUTH_PAYL_TO_CMD_OPCODE                 , (ke_msg_func_t)hci_rd_auth_payl_to_cmd_handler},
    {HCI_WR_AUTH_PAYL_TO_CMD_OPCODE                 , (ke_msg_func_t)hci_wr_auth_payl_to_cmd_handler},
    {HCI_LE_CON_UPDATE_CMD_OPCODE                   , (ke_msg_func_t)hci_le_con_update_cmd_handler},
    {HCI_LE_RD_CHNL_MAP_CMD_OPCODE                  , (ke_msg_func_t)hci_le_rd_chnl_map_cmd_handler},
    {HCI_LE_RD_REM_USED_FEATS_CMD_OPCODE            , (ke_msg_func_t)hci_le_rd_rem_used_feats_cmd_handler},
    {HCI_LE_START_ENC_CMD_OPCODE                    , (ke_msg_func_t)hci_le_start_enc_cmd_handler},
    {HCI_LE_LTK_REQ_REPLY_CMD_OPCODE                , (ke_msg_func_t)hci_le_ltk_req_reply_cmd_handler},
    {HCI_LE_LTK_REQ_NEG_REPLY_CMD_OPCODE            , (ke_msg_func_t)hci_le_ltk_req_neg_reply_cmd_handler},
    #if !(BLE_QUALIF)
    {HCI_LE_REM_CON_PARAM_REQ_REPLY_CMD_OPCODE      , (ke_msg_func_t)hci_le_rem_con_param_req_reply_cmd_handler},
    {HCI_LE_REM_CON_PARAM_REQ_NEG_REPLY_CMD_OPCODE  , (ke_msg_func_t)hci_le_rem_con_param_req_neg_reply_cmd_handler},
    {HCI_LE_SET_DATA_LEN_CMD_OPCODE                 , (ke_msg_func_t)hci_le_set_data_len_cmd_handler},
    #if(BLE_2MBPS)
    {HCI_LE_RD_PHY_CMD_OPCODE                       , (ke_msg_func_t)hci_le_rd_phy_cmd_handler},
    {HCI_LE_SET_PHY_CMD_OPCODE                      , (ke_msg_func_t)hci_le_set_phy_cmd_handler},
    #endif // (BLE_2MBPS)
    #endif // !(BLE_QUALIF)
    #if (BLE_TESTER)
    {HCI_TESTER_SET_LE_PARAMS_CMD_OPCODE            , (ke_msg_func_t)hci_tester_set_le_params_cmd_handler},
    {HCI_DBG_BLE_TST_LLCP_PT_EN_CMD_OPCODE          , (ke_msg_func_t)hci_dbg_ble_tst_llcp_pt_en_cmd_handler},
    {HCI_DBG_BLE_TST_SEND_LLCP_CMD_OPCODE           , (ke_msg_func_t)hci_dbg_ble_tst_send_llcp_cmd_handler},
    #endif
};
 ...
 ****************************************************************************************
 */
int llc_hci_command_handler(ke_msg_id_t const msgid,
                        void const *param,
                        ke_task_id_t const dest_id,
                        ke_task_id_t const src_id)
{
    ...
    for(int i = 0; i < (sizeof(llc_hci_command_handler_tab)/sizeof(llc_hci_command_handler_tab[0])); i++)
    {
 ...
 ****************************************************************************************
 */
int llc_hci_acl_data_tx_handler(ke_msg_id_t const msgid,
