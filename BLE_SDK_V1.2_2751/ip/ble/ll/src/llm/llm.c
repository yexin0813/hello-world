...
/// LLM task descriptor
static const struct ke_task_desc TASK_DESC_LLM = {NULL, &llm_default_handler, llm_state, LLM_STATE_MAX, LLM_IDX_MAX};
...
static void llm_wlpub_addr_set(uint16_t elem_index, struct bd_addr const *bdaddr)
{
    ...
    for (int i = 0; i < BLE_WLPUB_COUNT; i++)
    {
        ble_wlpub_set(elem_index, i, co_read16p(&bdaddr->addr[2 * i]));
    }
}
...
{
    // store the address at the desired index
    for (int i = 0; i < BLE_WLPRV_COUNT; i++)
    {
        ble_wlprv_set(elem_index, i, co_read16p(&bdaddr->addr[2 * i]));
    }
}
...
static void llm_adv_report_send(uint8_t rx_hdl, uint8_t adv_type)
{
                ...
                ))
    {
        ...
        {
            struct hci_le_dir_adv_rep_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, 0, 0, hci_le_dir_adv_rep_evt);
    ...
    else if (llm_util_check_evt_mask(LE_ADV_REP_EVT_BIT))
    {
        ...
        struct hci_le_adv_report_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, 0, 0, hci_le_adv_report_evt);
        // gets event subcode
...
void llm_init(bool reset)
{
    ...
    if (!reset)
    {
        ...
        ke_event_callback_set(KE_EVENT_BLE_CRYPT, &llm_encryption_done);
    }
    else
    {
        ...
        if (llm_le_env.advertising_params)
        {
            ...
            ke_free(llm_le_env.advertising_params);
        }
        ...
        {
            ke_free(llm_le_env.scanning_params);
        }
    ...
    llm_le_env.aa.intrand = co_rand_byte();
    ...
    co_list_init(&llm_le_env.adv_list);
    #endif //(BLE_OBSERVER || BLE_CENTRAL)
    ...
    co_write32p (&llm_le_env.public_add.addr[0],co_btohl(ble_bdaddrl_get()));
    co_write16p (&llm_le_env.public_add.addr[4],co_btohs(ble_bdaddru_get()));

    // Free the list of queued encryption requests
    while (1)
    {
        // Pop an encryption request from the list
        struct ke_msg *msg = (struct ke_msg *)co_list_pop_front(&llm_le_env.enc_req);
    ...
    co_list_init(&llm_le_env.enc_req);
    // Initialize the list of device(s) connected
    #if (BLE_PERIPHERAL || BLE_CENTRAL)
    co_list_init(&llm_le_env.cnx_list);
...
void llm_ble_ready(void)
{
    ...
    llm_common_cmd_complete_send(HCI_NO_OPERATION_CMD_OPCODE, CO_ERROR_NO_ERROR);
}
...
void llm_con_req_ind(uint8_t rx_hdl, uint16_t status)
{
    ...
    do
    {
        ...
        if(llc_util_get_free_conhdl(&conhdl) != CO_ERROR_NO_ERROR)
        {
            break;
        }
        ...
        if(llm_util_bl_check((struct bd_addr *)&data.inita, param.peer_addr_type,
                             &conhdl, LLM_UTIL_BL_NO_ACTION_WL, NULL) == CO_ERROR_NO_ERROR)
        {
...
void llm_common_cmd_complete_send(uint16_t opcode, uint8_t status)
{
    ...
    struct hci_basic_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, opcode, hci_basic_cmd_cmp_evt);
    // update the status
    event->status = status;
    // send the message
    hci_send_2_host(event);
...
{
    // allocate the status event message
    struct hci_cmd_stat_event *event = KE_MSG_ALLOC(HCI_CMD_STAT_EVENT, 0, opcode, hci_cmd_stat_event);
    // update the status
    event->status = status;
    // send the message
    hci_send_2_host(event);
...
uint8_t llm_test_mode_start_tx(void const *param, ke_msg_id_t const msgid)
{
    ...
    if(tx_buff_node && tx_desc_node)
    {
        ...
        ke_state_set(TASK_LLM, LLM_TEST);
        ...
        if (msgid == HCI_LE_ENH_TX_TEST_CMD_OPCODE)
        {
                    ...
                    ||(tx_test_mode->phy > PHYS_2MBPS_PREF))
            {
                ...
                ke_state_set(TASK_LLM, LLM_IDLE);
                return (CO_ERROR_PARAM_OUT_OF_MAND_RANGE);
            }
...
uint8_t llm_set_adv_en(struct hci_le_set_adv_en_cmd const *param)
{
    ...
    if(param->adv_en)
    {
        ...
        if((co_bdaddr_compare((struct bd_addr *)&llm_le_env.rand_add.addr[0], &co_null_bdaddr) == true) &&
        ...
        ke_state_set(TASK_LLM, LLM_ADVERTISING);
...
{
    if(jump_table[HBEACON]) Hbeacon_trap(HBEACON_DATA_BACKUP,(void *)param,NULL,NULL);
...
uint8_t llm_set_scan_param(struct hci_le_set_scan_param_cmd const *param)
{
    ...
    if ((param->scan_window > param->scan_intv) || (param->scan_type > SCAN_ACTIVE) || (param->scan_window > LLM_SCAN_WINDOW_MAX) ||
    ...
    if(param->scan_type == SCAN_ACTIVE)
    {
        // prepare the scan response header
        ble_txphadv_pack(LLM_LE_SCAN_CON_REQ_ADV_DIR_IDX,   // Descriptor index
                12,                                         // Packet length
...
uint8_t llm_set_scan_en(struct hci_le_set_scan_en_cmd const *param)
{
    ...
    else
    {
        ...
        if((co_bdaddr_compare((struct bd_addr *)&llm_le_env.rand_add.addr[0], &co_null_bdaddr) == true)
                ...
                && (llm_le_env.scanning_params->type == SCAN_ACTIVE))
        {
            return  (CO_ERROR_INVALID_HCI_PARAM);
        }
        ...
        ke_state_set(TASK_LLM, LLM_SCANNING);
    }
...
uint8_t llm_wl_dev_add_hdl(struct bd_addr *bd_addr, uint8_t bd_addr_type)
{
    ...
    do
    {
        ...
        #if (BLE_CENTRAL || BLE_PERIPHERAL)
        // Check black list
        if(llm_util_bl_check(bd_addr, bd_addr_type, NULL, LLM_UTIL_BL_SET_WL, NULL) != CO_ERROR_ACL_CON_EXISTS)
        #endif // (BLE_CENTRAL || BLE_PERIPHERAL)
        {
...
uint8_t llm_create_con(struct hci_le_create_con_cmd const *param)
{
    ...
    do
    {
        ...
        if(llc_util_get_free_conhdl(&llm_le_env.conhdl_alloc) != CO_ERROR_NO_ERROR)
        {
            // Lack of resources
        ...
        {
            status = (llm_le_env.nb_dev_in_wl == 0) ? CO_ERROR_COMMAND_DISALLOWED : CO_ERROR_ACL_CON_EXISTS;
        }
        ...
        if(status == CO_ERROR_NO_ERROR)
        {
            ...
            ke_state_set(TASK_LLM, LLM_INITIATING);
...
void llm_encryption_done(void)
{
    ...
    ke_event_clear(KE_EVENT_BLE_CRYPT);
    ...
    if(KE_TYPE_GET(msg->src_id) != TASK_LLC)
    {
    ...
    {
        // Allocate the message for the response
        struct llm_enc_ind *ind = KE_MSG_ALLOC(LLM_ENC_IND, msg->src_id, TASK_LLM, llm_enc_ind);
    ...
    msg = (struct ke_msg *)co_list_pick(&llm_le_env.enc_req);
    if (msg !=  NULL)
    {
        // Start the encryption
        llm_encryption_start((struct llm_enc_req *)ke_msg2param(msg));
    }
    else
    {
        #if (DEEP_SLEEP)
...
void llm_encryption_start(struct llm_enc_req const *param)
{
    ...
    ble_aeskey31_0_set(co_read32p(&param->key.ltk[0]));
    ble_aeskey63_32_set(co_read32p(&param->key.ltk[4]));
    ble_aeskey95_64_set(co_read32p(&param->key.ltk[8]));
    ble_aeskey127_96_set(co_read32p(&param->key.ltk[12]));
...
uint8_t llm_ral_get_rpa(struct hci_le_rd_loc_rslv_addr_cmd *param, struct bd_addr * addr, bool local)
{
    ...
    else if(llm_util_bd_addr_in_ral(&(param->peer_id_addr), param->peer_id_addr_type, &position))
    {
        ...
        if((offset != 0) && (status == CO_ERROR_NO_ERROR))
        {
            // Do a burst write in the exchange memory
