 ...
 ****************************************************************************************
 */
static int hci_le_set_evt_mask_cmd_handler(ke_msg_id_t const msgid,
                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    llm_common_cmd_complete_send(src_id, CO_ERROR_NO_ERROR);
 ...
 ****************************************************************************************
 */
static int hci_le_set_rand_add_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_set_adv_param_cmd_handler(ke_msg_id_t const msgid,
 ...
 */
static int
hci_le_rd_adv_ch_tx_pw_cmd_handler(ke_msg_id_t const msgid,
        void const *param,
        ke_task_id_t const dest_id,
        ke_task_id_t const src_id)
{
    ...
    event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_LE_RD_ADV_CHNL_TX_PW_CMD_OPCODE, hci_rd_adv_chnl_tx_pw_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_le_set_adv_data_cmd_handler(ke_msg_id_t const msgid,
        ...
        case LLM_ADVERTISING:
        {
            ...
            else
            {
                ...
                if(llm_le_env.advertising_params->adv_data_req)
                {
                    ...
                    ke_msg_free(llm_le_env.advertising_params->adv_data_req);
                    llm_le_env.advertising_params->adv_data_req = NULL;
                }
                GLOBAL_INT_RESTORE();
                
                // Extract the ke_msg pointer from the param passed
                llm_le_env.advertising_params->adv_data_req = ke_param2msg(param);
 ...
 */
static int
hci_le_set_adv_en_cmd_handler(ke_msg_id_t const msgid,
        ...
        ke_task_id_t const src_id)
{
    ...
    if(status == CO_ERROR_NO_ERROR)
    {
        ...
        llm_le_env.state = ke_state_get(TASK_LLM);
    ...
    if((status != CO_ERROR_NO_ERROR)||(param->adv_en == ADV_EN))
    {
        // Send the command complete event
        llm_common_cmd_complete_send(src_id, status);
    }
 ...
 */
static int
hci_le_set_scan_rsp_data_cmd_handler(ke_msg_id_t const msgid,
    ...
    switch(ke_state_get(TASK_LLM))
    {
        ...
        case LLM_ADVERTISING:
        {
            ...
            else
            {
                ...
                {
                    // Free the stored message
                    ke_msg_free(llm_le_env.advertising_params->scan_rsp_req);
                    llm_le_env.advertising_params->scan_rsp_req = NULL;
                }
                GLOBAL_INT_RESTORE();

                // Extract the ke_msg pointer from the param passed
                llm_le_env.advertising_params->scan_rsp_req = ke_param2msg(param);
                msg_status = KE_MSG_NO_FREE;
            }
        }break;
        default:
 ...
 */
static int
hci_le_set_scan_param_cmd_handler(ke_msg_id_t const msgid,
 ...
 */
static int
hci_le_set_scan_en_cmd_handler(ke_msg_id_t const msgid,
        ...
        ke_task_id_t const src_id)
{
    ...
    if((status != CO_ERROR_NO_ERROR)||(param->scan_en == SCAN_EN))
    {
        // Send the command complete event
        llm_common_cmd_complete_send(src_id, status);
 ...
 ****************************************************************************************
 */
static int hci_le_wl_mngt_cmd_handler(ke_msg_id_t const msgid,
                                      void const *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_le_create_con_cmd_handler(ke_msg_id_t const msgid,
                                         struct hci_le_create_con_cmd const *param,
                                         ke_task_id_t const dest_id,
                                         ke_task_id_t const src_id)
{
    // status returned in the command complete event
    uint8_t status = (uint8_t) CO_ERROR_NO_ERROR;
 ...
 ****************************************************************************************
 */
static int hci_le_create_con_cancel_cmd_handler(ke_msg_id_t const msgid,
                                                void const *param,
                                                ke_task_id_t const dest_id,
                                                ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_le_set_host_ch_class_cmd_handler(ke_msg_id_t const msgid,
                                                ...
                                                ke_task_id_t const src_id)
{
    ...
    if (llm_le_env.ch_map_assess.llm_le_set_host_ch_class_cmd_sto == true)
    {
         ...
         * 1 sec --> 100*10ms
         */
        ke_timer_set(LLM_LE_SET_HOST_CH_CLASS_CMD_STO, TASK_LLM, 100);
 ...
 ****************************************************************************************
 */
static int hci_le_rd_local_supp_feats_cmd_handler(ke_msg_id_t const msgid,
                                                  void const *param,
                                                  ke_task_id_t const dest_id,
                                                  ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_le_rd_wl_size_cmd_handler(ke_msg_id_t const msgid,
                                         void const *param,
                                         ke_task_id_t const dest_id,
                                         ke_task_id_t const src_id)
{
    ...
    // allocate the complete event message
    event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_LE_RD_WLST_SIZE_CMD_OPCODE, hci_rd_wlst_size_cmd_cmp_evt);
    // update the status
    event->status = CO_ERROR_NO_ERROR;
    // update the status
 ...
 ****************************************************************************************
 */
static int hci_le_rd_supp_states_cmd_handler(ke_msg_id_t const msgid,
                                             void const *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id)
{
    ...
    // allocate the complete event message
    struct hci_rd_supp_states_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_LE_RD_SUPP_STATES_CMD_OPCODE, hci_rd_supp_states_cmd_cmp_evt);
...
static int hci_le_rd_buff_size_cmd_handler(ke_msg_id_t const msgid,
                                           void const *param,
                                           ke_task_id_t const dest_id,
                                           ke_task_id_t const src_id)
{
    ...
    // structure type for the complete command event
    struct hci_le_rd_buff_size_cmd_cmp_evt *event= KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_LE_RD_BUFF_SIZE_CMD_OPCODE, hci_le_rd_buff_size_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_le_enc_cmd_handler(ke_msg_id_t const msgid,
                                  ...
                                  ke_task_id_t const src_id)
{
    ...
    co_list_push_back(&llm_le_env.enc_req, &msg->hdr);
 ...
 ****************************************************************************************
 */
static int hci_le_rand_cmd_handler(ke_msg_id_t const msgid,
                                   void const *param,
                                   ke_task_id_t const dest_id,
                                   ke_task_id_t const src_id)
{
...
#if 0 // DEBUG ONLY
    {
        ...
        co_write32p(&event->nb.nb[0], dbg_rand_word);
        co_write32p(&event->nb.nb[4], dbg_rand_word);
    }
#else
 ...
 ****************************************************************************************
 */
static int hci_reset_cmd_handler(ke_msg_id_t const msgid,
                                 void const *param,
                                 ke_task_id_t const dest_id,
                                 ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_rd_bd_addr_cmd_handler(ke_msg_id_t const msgid,
                                      void const *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
    // allocate the Command Complete event message
    struct hci_rd_bd_addr_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_RD_BD_ADDR_CMD_OPCODE, hci_rd_bd_addr_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_rd_local_ver_info_cmd_handler(ke_msg_id_t const msgid,
                                             void const *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id)
{
    // allocate the Command Complete event message
    struct hci_rd_local_ver_info_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_RD_LOCAL_VER_INFO_CMD_OPCODE, hci_rd_local_ver_info_cmd_cmp_evt);

    // gets the hci version
    event->hci_ver = RWBT_SW_VERSION_MAJOR;
    // gets the hci revision
    event->hci_rev = CO_SUBVERSION_BUILD(RWBT_SW_VERSION_MINOR, RWBT_SW_VERSION_BUILD);
    // gets the lmp version
    event->lmp_ver = RWBT_SW_VERSION_MAJOR;
    // gets the lmp subversion
    event->lmp_subver = CO_SUBVERSION_BUILD(RWBT_SW_VERSION_MINOR,
 ...
 ****************************************************************************************
 */
static int hci_rd_local_supp_cmds_cmd_handler(ke_msg_id_t const msgid,
                                             void const *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id)
{
    // allocate the Command Complete event message
    struct hci_rd_local_supp_cmds_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_RD_LOCAL_SUPP_CMDS_CMD_OPCODE, hci_rd_local_supp_cmds_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_rd_local_supp_feats_cmd_handler(ke_msg_id_t const msgid, void const *param,
                                               ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // allocate the complete event message
    struct hci_rd_local_supp_feats_cmd_cmp_evt *evt = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, src_id, hci_rd_local_supp_feats_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_set_evt_mask_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_set_evt_mask_page_2_cmd_handler(ke_msg_id_t const msgid,
                                               struct hci_set_evt_mask_cmd const *param,
                                               ke_task_id_t const dest_id,
                                               ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_set_ctrl_to_host_flow_ctrl_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_host_buf_size_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_host_nb_cmp_pkts_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_rd_buff_size_cmd_handler(ke_msg_id_t const msgid,
                                        void const *param,
                                        ke_task_id_t const dest_id,
                                        ke_task_id_t const src_id)
{
    // structure type for the complete command event
    struct hci_rd_buff_size_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_RD_BUFF_SIZE_CMD_OPCODE, hci_rd_buff_size_cmd_cmp_evt);
 ...
 */
#if (BLE_PERIPHERAL || BLE_CENTRAL)
static int hci_le_rd_suggted_dft_data_len_cmd_handler(ke_msg_id_t const msgid,
                                                      void const *param,
                                                      ke_task_id_t const dest_id,
                                                      ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_le_wr_suggted_dft_data_len_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_rd_max_data_len_cmd_handler(ke_msg_id_t const msgid,
                                              void const *param,
                                              ke_task_id_t const dest_id,
                                              ke_task_id_t const src_id)
{
    // allocate the Command Complete event message
    struct hci_le_rd_max_data_len_cmd_cmp_evt *event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_LE_RD_MAX_DATA_LEN_CMD_OPCODE, hci_le_rd_max_data_len_cmd_cmp_evt);
 ...
 ****************************************************************************************
 */
static int hci_le_enh_privacy_mgmt_cmd_handler(ke_msg_id_t const msgid,
                                               void const *param,
                                               ke_task_id_t const dest_id,
                                               ke_task_id_t const src_id)
{
        ...
        switch (msgid)
        {
            case HCI_LE_SET_ADDR_RESOL_EN_CMD_OPCODE:
            {
                ...
                if(en_cmd->enable)
                {
                    ...
                    ke_timer_set(LLM_LE_ENH_PRIV_ADDR_RENEW_TIMER, TASK_LLM, KE_TIME_IN_SEC(llm_le_env.enh_priv_rpa_timeout));
                ...
                else
                {
                    ...
                    ke_timer_clear(LLM_LE_ENH_PRIV_ADDR_RENEW_TIMER, TASK_LLM);
                }
 ...
 ****************************************************************************************
 */
static int hci_le_enh_privacy_info_cmd_handler(ke_msg_id_t const msgid,
                                               void const *param,
                                               ke_task_id_t const dest_id,
                                               ke_task_id_t const src_id)
{
    switch (msgid)
    {
        case HCI_LE_RD_RSLV_LIST_SIZE_CMD_OPCODE:
        {
            ...
            // allocate the complete event message
            event = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, HCI_LE_RD_RSLV_LIST_SIZE_CMD_OPCODE, hci_le_rd_rslv_list_size_cmd_cmp_evt);
            // update the status
            event->status = CO_ERROR_NO_ERROR;
            // update the status
        ...
        case HCI_LE_RD_LOC_RSLV_ADDR_CMD_OPCODE:
        {
            ...
            // allocate the complete event message
            rd_cmp_evt = KE_MSG_ALLOC(HCI_CMD_CMP_EVENT, 0, msgid, hci_le_rd_peer_rslv_addr_cmd_cmp_evt);
...
static int hci_le_rd_local_p256_public_key_cmd_handler(ke_msg_id_t const msgid,
                                                       void const *param,
                                                       ke_task_id_t const dest_id,
                                                       ke_task_id_t const src_id)
{
...
static int hci_le_generate_dhkey_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_set_dft_phy_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_test_mode_mngt_cmd_handler(ke_msg_id_t const msgid,
                                      void const *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
...
static const struct ke_msg_handler llm_hci_command_handler_tab[] =
{
    /// low energy commands
    {HCI_LE_SET_EVT_MASK_CMD_OPCODE,            (ke_msg_func_t)hci_le_set_evt_mask_cmd_handler},
    #if (BLE_PERIPHERAL || BLE_CENTRAL)
    {HCI_LE_RD_BUFF_SIZE_CMD_OPCODE,            (ke_msg_func_t)hci_le_rd_buff_size_cmd_handler},
    #endif // (BLE_PERIPHERAL || BLE_CENTRAL)
    {HCI_LE_RD_LOCAL_SUPP_FEATS_CMD_OPCODE,     (ke_msg_func_t)hci_le_rd_local_supp_feats_cmd_handler},
    {HCI_LE_SET_RAND_ADDR_CMD_OPCODE,           (ke_msg_func_t)hci_le_set_rand_add_cmd_handler},

    #if (BLE_BROADCASTER || BLE_PERIPHERAL)
    {HCI_LE_SET_ADV_PARAM_CMD_OPCODE,           (ke_msg_func_t)hci_le_set_adv_param_cmd_handler},
    {HCI_LE_RD_ADV_CHNL_TX_PW_CMD_OPCODE,       (ke_msg_func_t)hci_le_rd_adv_ch_tx_pw_cmd_handler},
    {HCI_LE_SET_ADV_DATA_CMD_OPCODE,            (ke_msg_func_t)hci_le_set_adv_data_cmd_handler},
    {HCI_LE_SET_ADV_EN_CMD_OPCODE,              (ke_msg_func_t)hci_le_set_adv_en_cmd_handler},
    {HCI_LE_SET_SCAN_RSP_DATA_CMD_OPCODE,       (ke_msg_func_t)hci_le_set_scan_rsp_data_cmd_handler},
    #endif //(BLE_BROADCASTER || BLE_PERIPHERAL)

    #if (BLE_OBSERVER || BLE_CENTRAL)
    {HCI_LE_SET_SCAN_PARAM_CMD_OPCODE,          (ke_msg_func_t)hci_le_set_scan_param_cmd_handler},
    {HCI_LE_SET_SCAN_EN_CMD_OPCODE,             (ke_msg_func_t)hci_le_set_scan_en_cmd_handler},
    #endif //(BLE_CENTRAL || BLE_OBSERVER)

    #if (BLE_CENTRAL)
    {HCI_LE_CREATE_CON_CMD_OPCODE,              (ke_msg_func_t)hci_le_create_con_cmd_handler},
    {HCI_LE_CREATE_CON_CANCEL_CMD_OPCODE,       (ke_msg_func_t)hci_le_create_con_cancel_cmd_handler},
    {HCI_LE_SET_HOST_CH_CLASS_CMD_OPCODE,       (ke_msg_func_t)hci_le_set_host_ch_class_cmd_handler},
    #endif //(BLE_CENTRAL)

    {HCI_LE_RD_WLST_SIZE_CMD_OPCODE,            (ke_msg_func_t)hci_le_rd_wl_size_cmd_handler},
    {HCI_LE_CLEAR_WLST_CMD_OPCODE,              (ke_msg_func_t)hci_le_wl_mngt_cmd_handler},
    {HCI_LE_ADD_DEV_TO_WLST_CMD_OPCODE,         (ke_msg_func_t)hci_le_wl_mngt_cmd_handler},
    {HCI_LE_RMV_DEV_FROM_WLST_CMD_OPCODE,       (ke_msg_func_t)hci_le_wl_mngt_cmd_handler},
    {HCI_LE_ENC_CMD_OPCODE,                     (ke_msg_func_t)hci_le_enc_cmd_handler},
    {HCI_LE_RAND_CMD_OPCODE,                    (ke_msg_func_t)hci_le_rand_cmd_handler},
    {HCI_LE_RD_SUPP_STATES_CMD_OPCODE,          (ke_msg_func_t)hci_le_rd_supp_states_cmd_handler},
#if !(BLE_QUALIF)
    #if (BLE_PERIPHERAL || BLE_CENTRAL)
    {HCI_LE_RD_SUGGTED_DFT_DATA_LEN_CMD_OPCODE, (ke_msg_func_t)hci_le_rd_suggted_dft_data_len_cmd_handler},
    {HCI_LE_WR_SUGGTED_DFT_DATA_LEN_CMD_OPCODE, (ke_msg_func_t)hci_le_wr_suggted_dft_data_len_cmd_handler},
    {HCI_LE_RD_MAX_DATA_LEN_CMD_OPCODE,         (ke_msg_func_t)hci_le_rd_max_data_len_cmd_handler},
    #endif // (BLE_PERIPHERAL || BLE_CENTRAL)
#endif
    #if (BLE_TEST_MODE_SUPPORT)
    {HCI_LE_RX_TEST_CMD_OPCODE,                 (ke_msg_func_t)hci_le_test_mode_mngt_cmd_handler},
    {HCI_LE_TX_TEST_CMD_OPCODE,                 (ke_msg_func_t)hci_le_test_mode_mngt_cmd_handler},
    {HCI_LE_TEST_END_CMD_OPCODE,                (ke_msg_func_t)hci_le_test_mode_mngt_cmd_handler},
    #endif //(BLE_TEST_MODE_SUPPORT)

    /// Legacy commands
    #if !BT_EMB_PRESENT
    {HCI_RESET_CMD_OPCODE,                       (ke_msg_func_t)hci_reset_cmd_handler},
    {HCI_RD_BD_ADDR_CMD_OPCODE,                  (ke_msg_func_t)hci_rd_bd_addr_cmd_handler},
    {HCI_RD_LOCAL_VER_INFO_CMD_OPCODE,           (ke_msg_func_t)hci_rd_local_ver_info_cmd_handler},
    {HCI_RD_LOCAL_SUPP_CMDS_CMD_OPCODE,          (ke_msg_func_t)hci_rd_local_supp_cmds_cmd_handler},
    {HCI_RD_LOCAL_SUPP_FEATS_CMD_OPCODE,         (ke_msg_func_t)hci_rd_local_supp_feats_cmd_handler},
    {HCI_SET_CTRL_TO_HOST_FLOW_CTRL_CMD_OPCODE,  (ke_msg_func_t)hci_set_ctrl_to_host_flow_ctrl_cmd_handler},
    {HCI_SET_EVT_MASK_CMD_OPCODE,                (ke_msg_func_t)hci_set_evt_mask_cmd_handler},
    {HCI_SET_EVT_MASK_PAGE_2_CMD_OPCODE,         (ke_msg_func_t)hci_set_evt_mask_page_2_cmd_handler},
    {HCI_HOST_BUF_SIZE_CMD_OPCODE,               (ke_msg_func_t)hci_host_buf_size_cmd_handler},
    {HCI_HOST_NB_CMP_PKTS_CMD_OPCODE,            (ke_msg_func_t)hci_host_nb_cmp_pkts_cmd_handler},
    {HCI_RD_BUFF_SIZE_CMD_OPCODE,                (ke_msg_func_t)hci_rd_buff_size_cmd_handler},
    #endif // !BT_EMB_PRESENT
#if !(BLE_QUALIF)
    /// Privacy 1.2
    {HCI_LE_ADD_DEV_TO_RSLV_LIST_CMD_OPCODE,    (ke_msg_func_t)hci_le_enh_privacy_mgmt_cmd_handler},
    {HCI_LE_RMV_DEV_FROM_RSLV_LIST_CMD_OPCODE,  (ke_msg_func_t)hci_le_enh_privacy_mgmt_cmd_handler},
    {HCI_LE_CLEAR_RSLV_LIST_CMD_OPCODE,         (ke_msg_func_t)hci_le_enh_privacy_mgmt_cmd_handler},
    {HCI_LE_SET_ADDR_RESOL_EN_CMD_OPCODE,       (ke_msg_func_t)hci_le_enh_privacy_mgmt_cmd_handler},
    {HCI_LE_SET_RSLV_PRIV_ADDR_TO_CMD_OPCODE,   (ke_msg_func_t)hci_le_enh_privacy_mgmt_cmd_handler},
    {HCI_LE_RD_PEER_RSLV_ADDR_CMD_OPCODE,       (ke_msg_func_t)hci_le_enh_privacy_info_cmd_handler},
    {HCI_LE_RD_LOC_RSLV_ADDR_CMD_OPCODE,        (ke_msg_func_t)hci_le_enh_privacy_info_cmd_handler},
    {HCI_LE_RD_RSLV_LIST_SIZE_CMD_OPCODE,       (ke_msg_func_t)hci_le_enh_privacy_info_cmd_handler},

#if (BLE_CENTRAL || BLE_PERIPHERAL)
    #if (SECURE_CONNECTIONS)
    /// BT 4.2 Secure Connections
    {HCI_LE_RD_LOC_P256_PUB_KEY_CMD_OPCODE,     (ke_msg_func_t)hci_le_rd_local_p256_public_key_cmd_handler},
    {HCI_LE_GEN_DHKEY_CMD_OPCODE,               (ke_msg_func_t)hci_le_generate_dhkey_cmd_handler},
    #endif // (SECURE_CONNECTIONS)
    #if(BLE_2MBPS)
    {HCI_LE_SET_DFT_PHY_CMD_OPCODE,             (ke_msg_func_t)hci_le_set_dft_phy_cmd_handler},
    {HCI_LE_ENH_RX_TEST_CMD_OPCODE,             (ke_msg_func_t)hci_le_test_mode_mngt_cmd_handler},
    {HCI_LE_ENH_TX_TEST_CMD_OPCODE,             (ke_msg_func_t)hci_le_test_mode_mngt_cmd_handler}
    #endif
#endif // (BLE_CENTRAL || BLE_PERIPHERAL)
#endif //#if !(BLE_QUALIF)
};
 ...
 ****************************************************************************************
 */
int hci_command_handler(ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    int return_status = KE_MSG_CONSUMED;

    // Check if there is a handler corresponding to the original command opcode
    for(int i = 0; i < (sizeof(llm_hci_command_handler_tab)/sizeof(llm_hci_command_handler_tab[0])); i++)
    {
