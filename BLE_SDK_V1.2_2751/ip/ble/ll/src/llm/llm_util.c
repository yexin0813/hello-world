...
uint16_t llm_util_bd_addr_wl_position(struct bd_addr const *bd_address, uint8_t bd_addr_type)
{
    ...
    if ((bd_addr_type & ADDR_MASK)  == ADDR_PUBLIC)
    {
        ...
        for(position = 0; position < BLE_WHITELIST_MAX ; position++)
        {
            ...
            if (co_bdaddr_compare(&tmp_addr, bd_address))
            {
                break;
            }
        }
...
bool llm_util_bd_addr_in_wl(struct bd_addr const *bd_address, uint8_t bd_addr_type, bool* in_black_list)
{
    ...
    if(llm_util_bl_check(bd_address, bd_addr_type, NULL, LLM_UTIL_BL_NO_ACTION_WL, &found) == CO_ERROR_ACL_CON_EXISTS)
    {
        if(in_black_list != NULL)
        {
            *in_black_list = true;
...
uint8_t llm_util_check_address_validity(struct bd_addr *bd_address, uint8_t addr_type)
{
    ...
    result = co_bdaddr_compare(bd_address, &co_null_bdaddr);
    if(result == true)
    {
...
void llm_util_adv_data_update(void)
{
    ...
    if (llm_le_env.advertising_params->adv_data_req)
    {
        ...
        llm_set_adv_data((struct hci_le_set_adv_data_cmd const *)ke_msg2param(llm_le_env.advertising_params->adv_data_req));
    ...
    {
        // Get the SET_ADV_DATA message
        llm_set_scan_rsp_data((struct hci_le_set_scan_rsp_data_cmd const *)ke_msg2param(llm_le_env.advertising_params->scan_rsp_req));

        // Free the stored message
        ke_msg_free(llm_le_env.advertising_params->scan_rsp_req);

        llm_le_env.advertising_params->scan_rsp_req = (struct ke_msg*)NULL;
    }
}
...
uint8_t llm_util_bl_check(const struct bd_addr *bd_addr_to_add, uint8_t bd_addr_type, uint16_t *conhdl, uint8_t wl_flag_action, bool* in_wl)
{
    ...
    struct llm_util_cnx_bd_addr_tag *scan = (struct llm_util_cnx_bd_addr_tag *)co_list_pick(&llm_le_env.cnx_list);
    ...
    while (scan)
    {
        ...
        else if(co_bdaddr_compare(bd_addr_to_add, &scan->dev_addr) && (scan->dev_addr_type == (bd_addr_type&ADDR_MASK)))
        {
            status = CO_ERROR_ACL_CON_EXISTS;
            if(conhdl != NULL)
            {
                *conhdl = scan->conhdl;
            }
...
uint8_t llm_util_bl_add(struct bd_addr *bd_addr_to_add, uint8_t bd_addr_type, uint16_t conhdl)
{
    ...
    struct llm_util_cnx_bd_addr_tag *elt = (struct llm_util_cnx_bd_addr_tag *)ke_malloc(sizeof(struct llm_util_cnx_bd_addr_tag), KE_MEM_ENV);
    uint8_t status = CO_ERROR_NO_ERROR;
    uint8_t position;
    ...
    if(elt)
    {
        ...
        co_list_push_back(&llm_le_env.cnx_list, &elt->hdr);
...
uint8_t llm_util_bl_rem(uint16_t conhdl)
{
    ...
    if(status == CO_ERROR_NO_ERROR)
    {
        ...
        co_list_extract(&llm_le_env.cnx_list, &scan->hdr, 0);
    ...
    *position = BLE_RESOL_ADDR_LIST_MAX;
    ...
    for(cursor = 0 ; cursor < BLE_RESOL_ADDR_LIST_MAX ; cursor++)
    {
        if(ble_entry_valid_getf(cursor))
        {
            ...
            // Compare the 2 BD addresses
            if (((bd_addr_type&ADDR_RAND) == ble_peer_id_type_getf(cursor)) && (co_bdaddr_compare(&tmp_addr, bd_address)))
            {
                *position = cursor;
                break;
            }
...
void llm_end_evt_defer(void)
{
        ...
        (!llm_le_env.advertising_params->adv_ldc_flag))
    {
        ...
        llc_le_con_cmp_evt_send(CO_ERROR_DIRECT_ADV_TO, 0, &msg_param);
