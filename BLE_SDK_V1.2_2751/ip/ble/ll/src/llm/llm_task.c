 ...
 ****************************************************************************************
 */
static int llm_enc_req_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int llm_ecc_result_ind_handler(ke_msg_id_t const msgid, struct ecc_result_ind const *param,
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    if (llm_le_env.cur_ecc_multiplication == LLM_DHKEY_GENERATION)
    {
        ...
        {
            struct hci_le_generate_dhkey_cmp_evt *event = KE_MSG_ALLOC(HCI_LE_EVENT, 0, HCI_LE_META_EVT_CODE,hci_le_generate_dhkey_cmp_evt);
 ...
 ****************************************************************************************
 */
static int llm_le_enh_priv_addr_renew_timer_handler(ke_msg_id_t const msgid, void const *param,
                                                    ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(ke_state_get(TASK_LLM) != LLM_IDLE)
    {
...
static int lld_stop_ind_handler(ke_msg_id_t const msgid,
                                ...
                                ke_task_id_t const src_id)
{
    if (ke_state_get(TASK_LLM) == LLM_STOPPING)
    {
        if( (llm_le_env.state == LLM_ADVERTISING) || (llm_le_env.state == LLM_SCANNING) || (llm_le_env.state == LLM_INITIATING))
        {
            ...
            llm_common_cmd_complete_send(llm_le_env.opcode, CO_ERROR_NO_ERROR);
            #if(BLE_CENTRAL)
            if(llm_le_env.state == LLM_INITIATING)
            {
                llc_le_con_cmp_evt_send(CO_ERROR_UNKNOWN_CONNECTION_ID, 0xFFFF, NULL);
            }
            #endif // (BLE_CENTRAL)
 ...
 ****************************************************************************************
 */
static int llm_dft_handler(ke_msg_id_t const msgid,
                           void *param,
                           ke_task_id_t const dest_id,
                           ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int llm_le_set_host_ch_class_cmd_sto_handler(ke_msg_id_t const msgid,
                                                    void const *param,
                                                    ke_task_id_t const dest_id,
                                                    ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
extern int hci_command_handler(ke_msg_id_t const msgid, void const *param, ke_task_id_t const dest_id, ke_task_id_t const src_id);
...
static const struct ke_msg_handler llm_default_state[] =
{
    // Note: first message is latest message checked by kernel so default is put on top.
    {KE_MSG_DEFAULT_HANDLER,                (ke_msg_func_t)llm_dft_handler},
    {LLD_STOP_IND,                          (ke_msg_func_t)lld_stop_ind_handler},
    {LLM_STOP_IND,                          (ke_msg_func_t)lld_stop_ind_handler},
    #if (BLE_CENTRAL)
    {LLM_LE_SET_HOST_CH_CLASS_CMD_STO,      (ke_msg_func_t)llm_le_set_host_ch_class_cmd_sto_handler},
    #endif // (BLE_CENTRAL)

    {LLM_ENC_REQ,                           (ke_msg_func_t)llm_enc_req_handler},

    #if SECURE_CONNECTIONS
    #if (BLE_CENTRAL || BLE_PERIPHERAL)
    {LLM_ECC_RESULT_IND,                    (ke_msg_func_t)llm_ecc_result_ind_handler},
    #endif //(BLE_CENTRAL || BLE_PERIPHERAL)
    #endif //SECURE_CONNECTIONS

    {LLM_LE_ENH_PRIV_ADDR_RENEW_TIMER,      (ke_msg_func_t)llm_le_enh_priv_addr_renew_timer_handler},
    {HCI_COMMAND,                           (ke_msg_func_t)hci_command_handler}
};

/// Specifies the message handlers that are common to all states.
const struct ke_state_handler llm_default_handler = KE_STATE_HANDLER(llm_default_state);

/// Defines the placeholder for the states of all the task instances.
ke_state_t llm_state[LLM_IDX_MAX];
