    ...
    ke_task_id_t const src_id)
{
    ...
    {
        ancc_enable_rsp_send(ancc_env,conidx,PRF_ERR_REQ_DISALLOWED);
    }
    ...
    ke_task_id_t const src_id)
{
    ...
    if(state == ANCC_IDLE)
    {
        prf_gatt_write_ntf_ind(&ancc_env->prf_env,conidx,ancc_env->env[conidx]->anc.descs[param->cfg].desc_hdl,PRF_CLI_START_NTF);
        if(param->cfg == ANC_DESC_NTF_SRC_CL_CFG)
        {
            ke_state_set(dest_id,ANCC_NTF_SRC_CL_CFG_NTF_ENABLING);
        }else
        {
            ke_state_set(dest_id,ANCC_DATA_SRC_CL_CFG_NTF_ENABLING);
        }
    }
    ...
    }else if(param->handle == ancc_env->env[conidx]->anc.chars[ANC_CHAR_DATA_SRC].val_hdl)
    {
        ...
        while(i<param->length)
        {
            ...
            case ATT_RSP_RX_APP_ID:
            {
                ...
                env->id.app_id.str[env->id.app_id.current_idx++]=param->value[i++];
            }
            break;
    ...
    ke_task_id_t const src_id)
{
    ...
    { 
        struct ancc_env_tag *ancc_env = PRF_ENV_GET(ANCC, ancc);
        uint8_t conidx = KE_IDX_GET(dest_id);
        uint8_t str_total_size = strlen((const char*)(param->app_id)) + 1;
        ...
        strcpy((void*)pkt->app_id_att_id,(void*)param->app_id);
