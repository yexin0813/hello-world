...
#include "../attm/attm_db.h" // Access to the internal database is required
    ...
    switch (req->flags)
    {
        case ATT_EXECUTE_ALL_PREPARED_WRITES:
            ...
            if(!co_list_is_empty(&(gattc_env[conidx]->server.prep_wr_req_list)))
            {
                ...
                struct gattc_write_req_ind *wr_ind = KE_MSG_ALLOC_DYN(GATTC_WRITE_REQ_IND,
                        atts_get_dest_task(conidx, ATTS_PREP_WRITE_GET(prep).handle), KE_BUILD_ID(TASK_GATTC, conidx),
