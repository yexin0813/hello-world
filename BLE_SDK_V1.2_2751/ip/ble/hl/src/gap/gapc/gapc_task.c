 ...
 ****************************************************************************************
 */
static int gapc_disconnect_cmd_handler(ke_msg_id_t const msgid, struct gapc_disconnect_cmd *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    switch(param->operation)
    {
        case GAPC_DISCONNECT:
        {
            ...
            {
                // set state to busy
                ke_state_set(KE_BUILD_ID(TASK_GAPC, idx), GAPC_DISC_BUSY);
 ...
 ****************************************************************************************
 */
static int gapc_connection_cfm_handler(ke_msg_id_t const msgid, struct gapc_connection_cfm *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int gapc_get_info_cmd_handler(ke_msg_id_t const msgid, struct gapc_get_info_cmd *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(msg_status == KE_MSG_NO_FREE)
    {
        ...
        switch(param->operation)
        {
            ...
            case GAPC_GET_PEER_FEATURES:
            {
                ...
                struct hci_le_rd_rem_used_feats_cmd *read_feat = KE_MSG_ALLOC(HCI_COMMAND, idx, HCI_LE_RD_REM_USED_FEATS_CMD_OPCODE, hci_le_rd_rem_used_feats_cmd);
            ...
            {
                /* allocate read rssi message */
                struct hci_basic_conhdl_cmd* rd_ch_map = KE_MSG_ALLOC(HCI_COMMAND, idx, HCI_LE_RD_CHNL_MAP_CMD_OPCODE, hci_basic_conhdl_cmd);
    ...
    if(state != GAPC_FREE)
    {
        switch(gapc_get_operation(conidx, GAPC_OP_LINK_INFO))
        {
            ...
            case GAPC_GET_PEER_SLV_PREF_PARAMS:
            {
                ...
                param_ind->info.slv_params.con_intv_min  = co_read16p(&(param->value[0]));
                param_ind->info.slv_params.con_intv_max  = co_read16p(&(param->value[2]));
                param_ind->info.slv_params.slave_latency = co_read16p(&(param->value[4]));
                param_ind->info.slv_params.conn_timeout  = co_read16p(&(param->value[6]));
 ...
 ****************************************************************************************
 */
static int gapc_param_update_cmd_handler(ke_msg_id_t const msgid, struct  gapc_param_update_cmd *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // list of handler supported operations
        ...
        else
        {
            ...
            else
            {
                struct l2cc_update_param_req *req = L2CC_SIG_PDU_ALLOC(idx, L2C_CODE_CONN_PARAM_UPD_REQ,
                                                                   KE_BUILD_ID(TASK_GAPC, idx), l2cc_update_param_req);
 ...
 ****************************************************************************************
 */
static int l2cc_pdu_recv_ind_handler(ke_msg_id_t const msgid, struct l2cc_pdu_recv_ind *ind,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int gapc_update_conn_param_to_ind_handler(ke_msg_id_t const msgid, void const *param,
                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
    ...
    if((state != GAPC_FREE) && (gapc_get_operation(conidx, GAPC_OP_LINK_UPD) == GAPC_UPDATE_PARAMS))
    {
        ...
        discon_req->reason = (uint8_t) CO_ERROR_UNACCEPTABLE_CONN_INT;
 ...
 ****************************************************************************************
 */
static int gapc_param_update_cfm_handler(ke_msg_id_t const msgid, struct gapc_param_update_cfm const *cfm,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
    ...
    if (state != GAPC_FREE)
    {
                ...
                && (GAPM_ISBITSET(gapc_env[idx]->features,GAPC_CONN_PARAM_REQ_FEAT_MASK)))
        {
            ...
            else
            {
                ...
                con_upd_reply->reason = CO_ERROR_UNACCEPTABLE_CONN_INT;
 ...
 ****************************************************************************************
 */
static int gapc_bond_cmd_handler(ke_msg_id_t const msgid, struct  gapc_bond_cmd *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // list of handler supported operations
 ...
 ****************************************************************************************
 */
static int gapc_bond_cfm_handler(ke_msg_id_t const msgid, struct gapc_bond_cfm *cfm,
                                 ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Returned message status
    int msg_status = KE_MSG_CONSUMED;
 ...
 ****************************************************************************************
 */
static int gapc_encrypt_cmd_handler(ke_msg_id_t const msgid, struct  gapc_encrypt_cmd *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // list of handler supported operations
 ...
 ****************************************************************************************
 */
static int gapc_encrypt_cfm_handler(ke_msg_id_t const msgid, struct gapc_encrypt_cfm *cfm,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Returned message status
    int msg_status = KE_MSG_CONSUMED;
 ...
 ****************************************************************************************
 */
static int gapc_security_cmd_handler(ke_msg_id_t const msgid, struct  gapc_security_cmd *param,
                   ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // list of handler supported operations
 ...
 ****************************************************************************************
 */
static int gapc_sign_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int l2cc_cmp_evt_handler(ke_msg_id_t const msgid, struct l2cc_cmp_evt *evt,
                                ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    #if (BLE_PERIPHERAL)
 ...
 ****************************************************************************************
 */
static int gapc_gen_dh_key_ind_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int gapc_key_press_notification_cmd_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int gapc_smp_timeout_timer_ind_handler(ke_msg_id_t const msgid,
                                          void const *param,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Recover connection index
 ...
 ****************************************************************************************
 */
static int gapc_smp_rep_attempts_timer_handler(ke_msg_id_t const msgid,
                                           void const *param,
                                           ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    if (ke_state_get(dest_id) != GAPC_FREE)
 ...
 ****************************************************************************************
 */
static int gattc_att_info_req_ind_handler(ke_msg_id_t const msgid, struct gattc_att_info_req_ind const *param,
                           ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int gattc_read_req_ind_handler(ke_msg_id_t const msgid, struct gattc_att_info_req_ind const *param,
                           ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current state
    uint8_t state = ke_state_get(dest_id);
    ...
    if(state != GAPC_FREE)
    {
        ...
        if(req != GAPC_DEV_INFO_MAX)
        {
            ...
            struct gapc_get_dev_info_req_ind * req_ind = KE_MSG_ALLOC(GAPC_GET_DEV_INFO_REQ_IND, APP_MAIN_TASK, dest_id, gapc_get_dev_info_req_ind);
            req_ind->req = req;
            ke_msg_send(req_ind);
        }
        else
 ...
 ****************************************************************************************
 */
static int gapc_get_dev_info_cfm_handler(ke_msg_id_t const msgid, struct gapc_get_dev_info_cfm const *param,
                           ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current state
    uint8_t state = ke_state_get(dest_id);
    ...
    if(state != GAPC_FREE)
    {
        ...
        switch(param->req)
        {
            ...
            case GAPC_DEV_SLV_PREF_PARAMS:
            {
                ...
                co_write16p(&param_cfm->value[cursor], param->info.slv_params.con_intv_min);
                cursor+= sizeof(uint16_t);
                co_write16p(&param_cfm->value[cursor], param->info.slv_params.con_intv_max);
                cursor+= sizeof(uint16_t);
                co_write16p(&param_cfm->value[cursor], param->info.slv_params.slave_latency);
                cursor+= sizeof(uint16_t);
                co_write16p(&param_cfm->value[cursor], param->info.slv_params.conn_timeout);
 ...
 ****************************************************************************************
 */
static int gapc_set_dev_info_cfm_handler(ke_msg_id_t const msgid, struct gapc_set_dev_info_cfm const *param,
                           ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int gapc_set_le_ping_to_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int gapc_set_le_pkt_size_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int gapc_set_phy_cmd_handler(ke_msg_id_t const msgid,
...
const struct ke_msg_handler gapc_default_state[] =
{
...
const struct ke_state_handler gapc_default_handler = KE_STATE_HANDLER(gapc_default_state);
