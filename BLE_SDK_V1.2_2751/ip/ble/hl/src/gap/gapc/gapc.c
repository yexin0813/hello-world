...
static const struct ke_task_desc TASK_DESC_GAPC  =
...
void gapc_update_state(uint8_t conidx, ke_state_t state, bool busy)
{
    ...
    {
        ke_state_t old_state  = ke_state_get(KE_BUILD_ID(TASK_GAPC, conidx));
        if(busy)
        {
            // set state to busy
            ke_state_set(KE_BUILD_ID(TASK_GAPC, conidx), old_state | state);
        }
        else
        {
            // set state to idle
