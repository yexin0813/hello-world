 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_cfg_handler(ke_msg_id_t const msgid, struct hci_basic_cmd_cmp_evt const *event,
 ...
 ****************************************************************************************
 */
static int hci_le_rd_buff_size_cmd_cmp_evt_handler(ke_msg_id_t const msgid, struct hci_le_rd_buff_size_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_rd_buff_size_cmd_cmp_evt_handler(ke_msg_id_t const msgid, struct hci_rd_buff_size_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ASSERT_ERR(event->status == CO_ERROR_NO_ERROR);
 ...
 ****************************************************************************************
 */
static int hci_no_operation_cmd_cmp_evt_handler(ke_msg_id_t const msgid, void const *param,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // inform host application that lower layers are ready but do nothing
    ke_msg_send_basic(GAPM_DEVICE_READY_IND, APP_MAIN_TASK, TASK_GAPM);
 ...
 ****************************************************************************************
 */
static int hci_rd_local_ver_info_cmd_cmp_evt_handler(ke_msg_id_t const msgid, struct hci_rd_local_ver_info_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
 ...
 ****************************************************************************************
 */
static int hci_rd_bd_addr_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
        struct hci_rd_bd_addr_cmd_cmp_evt const *event, ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    /* status should be OK */
    ASSERT_ERR(event->status == CO_ERROR_NO_ERROR);
...
static int hci_le_rd_local_p256_public_key_cmp_evt_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_generate_dhkey_cmp_evt_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if (ke_state_get(dest_id) != GAPM_DEVICE_SETUP)
    {
 ...
 ****************************************************************************************
 */
static int hci_le_rd_wlst_size_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
        ...
        else
        {
            ...
            if(param->status == CO_ERROR_NO_ERROR)
            {
                struct gapm_white_list_size_ind *wlist_size_ind =
                        KE_MSG_ALLOC(GAPM_WHITE_LIST_SIZE_IND, gapm_get_requester(GAPM_OP_CFG),
                                dest_id, gapm_white_list_size_ind);
                /* fill up the parameters */
                wlist_size_ind->size = param->wlst_size;
                /* send the message indication */
                ke_msg_send(wlist_size_ind);
            }
 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_wl_handler(ke_msg_id_t const msgid, struct hci_basic_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_adv_handler(ke_msg_id_t const msgid, struct hci_basic_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
    ...
    if(state != GAPM_DEVICE_SETUP)
    {
        ...
        if ((event->status == CO_ERROR_NO_ERROR) && (air_op != NULL))
        {
            switch (msgid)
            {
 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_scan_handler(ke_msg_id_t const msgid, struct hci_basic_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int hci_le_adv_report_evt_handler(ke_msg_id_t const msgid, struct hci_le_adv_report_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
    ...
    if(state != GAPM_DEVICE_SETUP)
    {
        ...
        for (uint8_t cursor = 0; (cursor < event->nb_reports) && (mode != GAP_INVALID_MODE); cursor++)
        {
            ...
            if(!filter)
            {
                ...
                struct gapm_adv_report_ind* report = KE_MSG_ALLOC(GAPM_ADV_REPORT_IND,
 ...
 ****************************************************************************************
 */
static int hci_le_dir_adv_report_evt_handler(ke_msg_id_t const msgid, struct hci_le_dir_adv_rep_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_connect_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_con_cmp_evt_handler(ke_msg_id_t const msgid, struct hci_le_con_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int hci_le_enh_con_cmp_evt_handler(ke_msg_id_t const msgid, struct hci_le_enh_con_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_addr_set_handler(ke_msg_id_t const msgid,
        struct hci_basic_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
            ...
            if ((event->status == CO_ERROR_NO_ERROR) && (air_op != NULL))
            {
                switch (msgid)
                {
                    ...
                    case HCI_LE_SET_RAND_ADDR_CMD_OPCODE:
                    {
                        ...
                        struct gapm_dev_bdaddr_ind *bdaddr_ind = KE_MSG_ALLOC(GAPM_DEV_BDADDR_IND,
 ...
 ****************************************************************************************
 */
static int hci_le_rd_adv_chnl_tx_pw_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_rand_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_enc_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
        ...
        switch (gapm_get_operation(GAPM_OP_CFG))
        {
            ...
            case (GAPM_GEN_RAND_ADDR):
            {
                ...
                if (param->status == GAP_ERR_NO_ERROR)
                {
                    ...
                    {
                        // Indication sent to the requester
                        struct gapm_dev_bdaddr_ind *ind = KE_MSG_ALLOC(GAPM_DEV_BDADDR_IND, gapm_get_requester(GAPM_OP_CFG),
 ...
 ****************************************************************************************
 */
static int hci_le_rd_suggted_dft_data_len_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_rd_max_data_len_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_le_rd_ral_size_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
        ...
        else
        {
            ...
            if(param->status == CO_ERROR_NO_ERROR)
            {
                struct gapm_ral_size_ind *ral_size_ind =
                        KE_MSG_ALLOC(GAPM_RAL_SIZE_IND, gapm_get_requester(GAPM_OP_CFG),
                                dest_id, gapm_ral_size_ind);
                /* fill up the parameters */
                ral_size_ind->size = param->size;
                /* send the message indication */
                ke_msg_send(ral_size_ind);
            }
 ...
 ****************************************************************************************
 */
static int hci_le_rd_ral_addr_cmd_cmp_evt_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int hci_basic_cmd_cmp_evt_rl_handler(ke_msg_id_t const msgid, struct hci_basic_cmd_cmp_evt const *event,
        ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    // Current process state
    uint8_t state = ke_state_get(dest_id);
...
static const struct ke_msg_handler gapm_hci_cmd_cmp_event_handler_tab[] =
{
    { HCI_RESET_CMD_OPCODE,                      (ke_msg_func_t) hci_basic_cmd_cmp_evt_cfg_handler },
    { HCI_SET_EVT_MASK_CMD_OPCODE,               (ke_msg_func_t) hci_basic_cmd_cmp_evt_cfg_handler },
    { HCI_LE_SET_EVT_MASK_CMD_OPCODE,            (ke_msg_func_t) hci_basic_cmd_cmp_evt_cfg_handler },

    #if (BLE_CENTRAL || BLE_PERIPHERAL)
    { HCI_LE_RD_BUFF_SIZE_CMD_OPCODE,            (ke_msg_func_t) hci_le_rd_buff_size_cmd_cmp_evt_handler },
    { HCI_RD_BUFF_SIZE_CMD_OPCODE,               (ke_msg_func_t) hci_rd_buff_size_cmd_cmp_evt_handler },
    #endif // (BLE_CENTRAL || BLE_PERIPHERAL)

    { HCI_NO_OPERATION_CMD_OPCODE,               (ke_msg_func_t) hci_no_operation_cmd_cmp_evt_handler },

    { HCI_LE_SET_HOST_CH_CLASS_CMD_OPCODE,       (ke_msg_func_t) hci_basic_cmd_cmp_evt_cfg_handler },

    { HCI_RD_LOCAL_VER_INFO_CMD_OPCODE,          (ke_msg_func_t) hci_rd_local_ver_info_cmd_cmp_evt_handler },
    { HCI_RD_BD_ADDR_CMD_OPCODE ,                (ke_msg_func_t) hci_rd_bd_addr_cmd_cmp_evt_handler },

    { HCI_LE_WR_SUGGTED_DFT_DATA_LEN_CMD_OPCODE, (ke_msg_func_t) hci_basic_cmd_cmp_evt_cfg_handler },
    { HCI_LE_RD_SUGGTED_DFT_DATA_LEN_CMD_OPCODE, (ke_msg_func_t) hci_le_rd_suggted_dft_data_len_cmd_cmp_evt_handler },
    { HCI_LE_RD_MAX_DATA_LEN_CMD_OPCODE,         (ke_msg_func_t) hci_le_rd_max_data_len_cmd_cmp_evt_handler },

    #if (BLE_2MBPS)
    { HCI_LE_SET_DFT_PHY_CMD_OPCODE,             (ke_msg_func_t) hci_basic_cmd_cmp_evt_cfg_handler },
    #endif // (BLE_2MBPS)

    #if (BLE_CENTRAL || BLE_PERIPHERAL)
    { HCI_LE_RD_WLST_SIZE_CMD_OPCODE,            (ke_msg_func_t) hci_le_rd_wlst_size_cmd_cmp_evt_handler },
    #endif //(BLE_CENTRAL || BLE_PERIPHERAL)
    { HCI_LE_ADD_DEV_TO_WLST_CMD_OPCODE,         (ke_msg_func_t) hci_basic_cmd_cmp_evt_wl_handler },
    { HCI_LE_RMV_DEV_FROM_WLST_CMD_OPCODE,       (ke_msg_func_t) hci_basic_cmd_cmp_evt_wl_handler },
    { HCI_LE_CLEAR_WLST_CMD_OPCODE,              (ke_msg_func_t) hci_basic_cmd_cmp_evt_wl_handler },

    /* Advertising procedure */
    #if (BLE_PERIPHERAL || BLE_BROADCASTER)
    { HCI_LE_SET_ADV_PARAM_CMD_OPCODE,           (ke_msg_func_t) hci_basic_cmd_cmp_evt_adv_handler },
    { HCI_LE_SET_ADV_DATA_CMD_OPCODE,            (ke_msg_func_t) hci_basic_cmd_cmp_evt_adv_handler },
    { HCI_LE_SET_SCAN_RSP_DATA_CMD_OPCODE,       (ke_msg_func_t) hci_basic_cmd_cmp_evt_adv_handler },
    { HCI_LE_SET_ADV_EN_CMD_OPCODE,              (ke_msg_func_t) hci_basic_cmd_cmp_evt_adv_handler },
    #endif // (BLE_PERIPHERAL || BLE_BROADCASTER)

    /* Scan procedure */
    #if (BLE_CENTRAL || BLE_OBSERVER)
    { HCI_LE_SET_SCAN_PARAM_CMD_OPCODE,          (ke_msg_func_t) hci_basic_cmd_cmp_evt_scan_handler },
    { HCI_LE_SET_SCAN_EN_CMD_OPCODE,             (ke_msg_func_t) hci_basic_cmd_cmp_evt_scan_handler },
    #endif // (BLE_CENTRAL || BLE_OBSERVER)

    /* Connection procedure */
    #if (BLE_CENTRAL)
    { HCI_LE_CREATE_CON_CANCEL_CMD_OPCODE,       (ke_msg_func_t) hci_basic_cmd_cmp_evt_connect_handler },
    #endif // (BLE_CENTRAL)

    /* Address Management */
    { HCI_LE_SET_RAND_ADDR_CMD_OPCODE,           (ke_msg_func_t) hci_basic_cmd_cmp_evt_addr_set_handler },

    #if ((BLE_OBSERVER) || (BLE_PERIPHERAL))
    /* retrieve adv tx power level value */
    { HCI_LE_RD_ADV_CHNL_TX_PW_CMD_OPCODE,       (ke_msg_func_t) hci_le_rd_adv_chnl_tx_pw_cmd_cmp_evt_handler },
    #endif // ((BLE_OBSERVER) || (BLE_PERIPHERAL))

    {HCI_LE_ENC_CMD_OPCODE,                      (ke_msg_func_t)hci_le_enc_cmd_cmp_evt_handler},
    {HCI_LE_RAND_CMD_OPCODE,                     (ke_msg_func_t)hci_le_rand_cmd_cmp_evt_handler},

    /* Resolving List Management */
    { HCI_LE_RD_RSLV_LIST_SIZE_CMD_OPCODE,       (ke_msg_func_t) hci_le_rd_ral_size_cmd_cmp_evt_handler },
    { HCI_LE_RD_PEER_RSLV_ADDR_CMD_OPCODE,       (ke_msg_func_t) hci_le_rd_ral_addr_cmd_cmp_evt_handler },
    { HCI_LE_RD_LOC_RSLV_ADDR_CMD_OPCODE,        (ke_msg_func_t) hci_le_rd_ral_addr_cmd_cmp_evt_handler },
    { HCI_LE_SET_ADDR_RESOL_EN_CMD_OPCODE,       (ke_msg_func_t) hci_basic_cmd_cmp_evt_rl_handler },
    { HCI_LE_SET_RSLV_PRIV_ADDR_TO_CMD_OPCODE,   (ke_msg_func_t) hci_basic_cmd_cmp_evt_rl_handler },
    { HCI_LE_ADD_DEV_TO_RSLV_LIST_CMD_OPCODE,    (ke_msg_func_t) hci_basic_cmd_cmp_evt_rl_handler },
    { HCI_LE_RMV_DEV_FROM_RSLV_LIST_CMD_OPCODE,  (ke_msg_func_t) hci_basic_cmd_cmp_evt_rl_handler },
    { HCI_LE_CLEAR_RSLV_LIST_CMD_OPCODE,         (ke_msg_func_t) hci_basic_cmd_cmp_evt_rl_handler },
};

#if (BLE_CENTRAL)
/// The message handlers for HCI status event
static const struct ke_msg_handler  gapm_hci_cmd_stat_event_handler_tab[] =
{
    { HCI_LE_CREATE_CON_CMD_OPCODE,              (ke_msg_func_t) hci_basic_cmd_cmp_evt_connect_handler },
};
#endif // (BLE_CENTRAL)

#if (BLE_CENTRAL || BLE_OBSERVER || BLE_PERIPHERAL)
/// The message handlers for HCI LE events
static const struct ke_msg_handler gapm_hci_le_event_handler_tab[] =
{
    #if (BLE_CENTRAL || BLE_OBSERVER)
    { HCI_LE_ADV_REPORT_EVT_SUBCODE,    (ke_msg_func_t) hci_le_adv_report_evt_handler },
    { HCI_LE_DIR_ADV_REP_EVT_SUBCODE,   (ke_msg_func_t) hci_le_dir_adv_report_evt_handler },
    #endif //(BLE_CENTRAL || BLE_OBSERVER)
    #if (BLE_CENTRAL || BLE_PERIPHERAL)
    { HCI_LE_CON_CMP_EVT_SUBCODE,           (ke_msg_func_t) hci_le_con_cmp_evt_handler },
    { HCI_LE_ENH_CON_CMP_EVT_SUBCODE,       (ke_msg_func_t) hci_le_enh_con_cmp_evt_handler },
    #if (SECURE_CONNECTIONS)
    { HCI_LE_RD_LOC_P256_PUB_KEY_CMP_EVT_SUBCODE,   (ke_msg_func_t)hci_le_rd_local_p256_public_key_cmp_evt_handler },
    { HCI_LE_GEN_DHKEY_CMP_EVT_SUBCODE,             (ke_msg_func_t)hci_le_generate_dhkey_cmp_evt_handler },// hci_generate_dhkey_cmd_cmp_evt_handler },
    #endif //  (SECURE_CONNECTIONS)
    #endif // (BLE_CENTRAL || BLE_PERIPHERAL)
};
 ...
 ****************************************************************************************
 */
int gapm_hci_handler(ke_msg_id_t const msgid, void const* event, ke_task_id_t dest_id, ke_task_id_t src_id)
{
