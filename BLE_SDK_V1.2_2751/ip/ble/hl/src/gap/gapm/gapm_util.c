...
void gapm_execute_scan_op(void)
{
    ...
    switch(GAPM_GET_OP_STATE(scan->op))
    {
        ...
        case GAPM_OP_SET_PARAMS:
        {
            struct hci_le_set_scan_param_cmd *scan_param = KE_MSG_ALLOC(HCI_COMMAND, 0, HCI_LE_SET_SCAN_PARAM_CMD_OPCODE, hci_le_set_scan_param_cmd);
...
uint8_t gapm_connect_op_sanity(void)
{
    ...
    {
        struct ke_msg* src_msg = ke_param2msg(connect);
        // ensure that at least one bd address is present in memory (will be used later)
        struct gapm_start_connection_cmd* new_cmd = KE_MSG_ALLOC_DYN(GAPM_START_CONNECTION_CMD,
    ...
    switch(GAPM_GET_OP_STATE(connect->op))
    {
        ...
        case GAPM_OP_START:
        {
            ...
            if(GAPM_IS_OP_FIELD_SET(connect->op, CONNECTING))
            {
                ...
                struct hci_le_create_con_cmd *conn_par = KE_MSG_ALLOC(HCI_COMMAND, 0, HCI_LE_CREATE_CON_CMD_OPCODE, hci_le_create_con_cmd);
...
{
    struct hci_le_set_adv_en_cmd *adv_en = KE_MSG_ALLOC(HCI_COMMAND, 0, HCI_LE_SET_ADV_EN_CMD_OPCODE, hci_le_set_adv_en_cmd);
...
{
    struct hci_le_set_scan_en_cmd *scan_en = KE_MSG_ALLOC(HCI_COMMAND, 0, HCI_LE_SET_SCAN_EN_CMD_OPCODE, hci_le_set_scan_en_cmd);
...
void gapm_setup_conn(ke_msg_id_t const msgid, struct hci_le_enh_con_cmp_evt const *event)
{
    ...
    if(event->status == CO_ERROR_NO_ERROR)
    {
        ...
        if(conidx == GAP_INVALID_CONIDX)
        {
            ...
            struct hci_disconnect_cmd *cmd = KE_MSG_ALLOC(HCI_COMMAND, event->conhdl, HCI_DISCONNECT_CMD_OPCODE, hci_disconnect_cmd);
            cmd->conhdl = event->conhdl;
            cmd->reason = CO_ERROR_REMOTE_DEV_TERM_LOW_RESOURCES;
    ...
    #if (BLE_CENTRAL)
    else if(event->status == CO_ERROR_UNKNOWN_CONNECTION_ID)
    {
        new_state = GAPM_OP_STOP;
    }
    #endif // (BLE_CENTRAL)
    #if (BLE_PERIPHERAL)
    else if(event->status == CO_ERROR_DIRECT_ADV_TO)
    {
    ...
    else
    {
        switch(current_state)
        {
            ...
            case GAPM_OP_SETUP_WR_LE_DFT_DATA_LEN_CMD:
            {
                ...
                gapm_env.renew_dur =  co_max(set_cfg->renew_dur, GAP_TMR_PRIV_ADDR_INT);
                ...
                switch(set_cfg->addr_type)
                {
                    ...
                    case GAPM_CFG_ADDR_CTNL_PRIVACY:
                    {
                        ...
                        struct hci_le_set_rslv_priv_addr_to_cmd *set_to = KE_MSG_ALLOC(HCI_COMMAND, 0,
            ...
            case GAPM_OP_SETUP_SET_RENEW_TO:
            {
                ...
                struct hci_le_set_addr_resol_en_cmd *en_addr_resol = KE_MSG_ALLOC(HCI_COMMAND, 0,
