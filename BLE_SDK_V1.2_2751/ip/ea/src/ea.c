...
void ea_elt_cancel(struct ea_elt_tag *new_elt)
{
    ...
    struct ea_elt_tag* cancel_curr = (struct ea_elt_tag*) co_list_pick(&ea_env.elt_canceled);
    ...
    while(cancel_curr)
    {
        ...
        struct ea_elt_tag* cancel_next = (struct ea_elt_tag*) co_list_next(&cancel_curr->hdr);
        ...
        if( (EA_ASAP_STG_TYPE_GET(cancel_curr) != EA_FLAG_NO_ASAP) && (resched_att > 0) )
        {
            ...
            while (wait_curr)
            {
                ...
                else
                {
                    ...
                    {
                        new_timestamp = CO_ALIGN2_LO(new_timestamp) + 1;
                    }
                    else if(parity == EA_EVEN_SLOT)
                    {
                        new_timestamp = CO_ALIGN2_HI(new_timestamp);
                    }
                ...
                wait_prev = wait_curr;
                wait_curr = (struct ea_elt_tag *) co_list_next(&wait_curr->hdr);
            }

            if(status == EA_ERROR_OK)
            {
                // Remove from cancel list
                co_list_extract_after(&ea_env.elt_canceled, &cancel_prev->hdr, &cancel_curr->hdr);

                // Place in wait list
                co_list_insert_after(&ea_env.elt_wait, &wait_prev->hdr, &cancel_curr->hdr);
            }
            else
            {
    ...
    if (!co_list_is_empty(&ea_env.elt_canceled))
    {
        #if BT_EMB_PRESENT
...
static void ea_prog_timer(void)
{
    ...
    struct ea_elt_tag *first_elt = (struct ea_elt_tag *)co_list_pick(&ea_env.elt_wait);
    ...
    {
        // Check the 1st alarm
        struct ea_alarm_tag *alarm = (struct ea_alarm_tag *) co_list_pick(&ea_env.alarm_list);
        if(alarm != NULL)
        {
...
{
    co_list_init(&ea_env.elt_wait);
    co_list_init(&ea_env.elt_canceled);
    co_list_init(&ea_env.interval_list);
    #if (EA_ALARM_SUPPORT)
    co_list_init(&ea_env.alarm_list);
    #endif //(EA_ALARM_SUPPORT)
...
{
    // Create element
    struct ea_elt_tag *elt = (struct ea_elt_tag *)ke_malloc((sizeof(struct ea_elt_tag) + size_of_env),
            KE_MEM_ENV);
...
uint8_t ea_elt_insert (struct ea_elt_tag *elt)
{
    ...
    struct co_list *list = &ea_env.elt_wait;
    ...
    for (;;)
    {
        ...
        if(EA_ASAP_STG_TYPE_GET(elt) != EA_FLAG_NO_ASAP)
        {
            ...
            if(parity == EA_ODD_SLOT)
            {
                first_allowed_ts = (CO_ALIGN2_LO(first_allowed_ts) + 1) & MAX_SLOT_CLOCK;
            }
            else if(parity == EA_EVEN_SLOT)
            {
                first_allowed_ts = CO_ALIGN2_HI(first_allowed_ts) & MAX_SLOT_CLOCK;
            }
        }
        ...
        while (scan)
        {
            ...
            if (scan != ea_env.elt_prog)
            {
                ...
                scan = (struct ea_elt_tag *) co_list_next(&scan->hdr);
            }
            else // If the scanned element was the programmed one, check the wait list
            {
                scan = (struct ea_elt_tag *) co_list_pick(list);
            }
        }
        break;
    }
    ...
    if (status != EA_ERROR_REJECTED)
    {
        if(cancel_cpt)
        {
            ...
            co_list_extract(&ea_env.elt_wait, &cancel->hdr, cancel_cpt-1);
            ...
            while(cancel_cpt > 0)
            {
                ...
                co_list_push_back(&ea_env.elt_canceled, &cancel->hdr);
        ...
        {
            // Second or more
            co_list_insert_after(&ea_env.elt_wait,&prev->hdr,&elt->hdr);
        }
        else
        {
            // First
            co_list_push_front(&ea_env.elt_wait,&elt->hdr);
...
uint8_t ea_elt_remove(struct ea_elt_tag *elt)
{
    ...
    if (elt)
    {
        ...
        do
        {
            ...
            if (&elt->hdr == co_list_pick(&ea_env.elt_wait))
            {
                // Pop the first element of the list
                co_list_pop_front(&ea_env.elt_wait);
            ...
            if(co_list_extract(&ea_env.elt_wait, &elt->hdr, 0))
                break;

            // Try extracting the element from the cancel queue
            if(co_list_extract(&ea_env.elt_canceled, &elt->hdr, 0))
                break;
...
{
    struct ea_interval_tag *new_intv = (struct ea_interval_tag *)ke_malloc(sizeof(struct ea_interval_tag), KE_MEM_ENV);
...
{
    co_list_push_back(&ea_env.interval_list,&interval_to_add->hdr);
}

void ea_interval_remove(struct ea_interval_tag *interval_to_remove)
{
    // Extract the interval linked to this element
    co_list_extract(&ea_env.interval_list, &interval_to_remove->hdr, 0);
}
...
void ea_finetimer_isr(void)
{
    ...
    #if (EA_ALARM_SUPPORT)
    {
        ...
        while(alarm != NULL)
        {
            ...
            {
                // Save next
                alarm_next = (struct ea_alarm_tag*) co_list_next((struct co_list_hdr *) alarm);

                // Pop the alarm
                co_list_pop_front(&ea_env.alarm_list);
...
void ea_sw_isr(void)
{
    ...
    while (!co_list_is_empty(&ea_env.elt_canceled))
    {
        elt = (struct ea_elt_tag *)co_list_pop_front(&ea_env.elt_canceled);
...
uint8_t ea_offset_req(struct ea_param_input* input_param, struct ea_param_output* output_param)
{
    ...
    scan = (struct ea_interval_tag *) co_list_pick(&ea_env.interval_list);
...
void ea_interval_duration_req(struct ea_param_input* input_param, struct ea_param_output* output_param)
{
    ...
    if (input_param->interval_min < input_param->interval_max)
    {
        ...
        uint8_t nb_links = 0;
        struct ea_interval_tag *scan = (struct ea_interval_tag *) co_list_pick(&ea_env.interval_list);
    ...
    output_param->duration = co_max(input_param->duration_min, EA_BW_USED_DFT_SLOT);
}
...
uint8_t ea_alarm_set(struct ea_alarm_tag* elt)
{
    ...
    if(CLK_DIFF(current_time, elt->timestamp) >= EA_TIMER_PROG_DELAY)
    {
        ...
        struct ea_alarm_tag* scan_next = (struct ea_alarm_tag*) co_list_pick(&ea_env.alarm_list);
        ...
        while(scan_next != NULL)
        {
            ...
            scan_prev = scan_next;
            scan_next = (struct ea_alarm_tag*) co_list_next((struct co_list_hdr*) scan_next);
        }
        ...
        if(scan_prev == NULL)
        {
            ...
            co_list_push_front(&ea_env.alarm_list, (struct co_list_hdr*) elt);
        ...
        else if(scan_next == NULL)
        {
            ...
            co_list_push_back(&ea_env.alarm_list, (struct co_list_hdr*) elt);
        }
        else
        {
            // Insert between previous and next
            scan_prev->hdr.next = (struct co_list_hdr*) elt;
            elt->hdr.next = (struct co_list_hdr*) scan_next;
        }
...
uint8_t ea_alarm_clear(struct ea_alarm_tag* elt)
{
    ...
    do
    {
        if(((struct co_list_hdr*)elt) == co_list_pick(&ea_env.alarm_list))
        {
            // Pop the first element of the list
            co_list_pop_front(&ea_env.alarm_list);
        ...
        if(co_list_extract(&ea_env.alarm_list, (struct co_list_hdr*) elt, 0))
            break;
